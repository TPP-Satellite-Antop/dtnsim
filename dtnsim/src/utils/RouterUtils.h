...
typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, RouterVertexInfo,
                              RouterEdgeInfo, RouterGraphInfo>
    RouterGraph;
...
map<double, RouterGraph> computeFlows(ContactPlan *contactPlan, int nodesNumber,
                                      string bundleMapsLocation);

/// @brief Save Flows in dot and pdf files located in outFileLocation
void saveGraphs(ContactPlan *contactPlan, map<double, RouterGraph> *flows, vector<string> dotColors,
                map<pair<int, int>, unsigned int> flowIds, std::string outFileLocation);
