    ...
    for (; ic1 != ic2; ++ic1) {
        ...
        for (size_t ii1 = 0; ii1 < stateTimes.size() - 1; ++ii1) {
            ...
            if (stateStart >= contactStart && stateStart < contactEnd) {
                ...
                TopologyGraph::edge_descriptor edge =
                    (add_edge(vertexSource, vertexDestination, topGraph)).first;
...
void saveGraphs(map<double, TopologyGraph> *topology, std::string outFileLocation) {
    ...
    for (; it1 != it2; ++it1) {
        ...
        ofs << vertexId << "." << state << " [shape=box,fontsize=16,label=" << labelString << "];"
            << endl;
        ...
        for (tie(vi, vi_end) = vertices(graph); vi != vi_end; ++vi) {
            for (tie(vi2, vi2_end) = vertices(graph); vi2 != vi2_end; ++vi2) {
                if (vi2 != vi && *vi2 > *vi) {
                    ...
                    ofs << v1Id << "." << state << edgeString << v2Id << "." << state
                        << " [style=\"invis\"];" << endl;
                    break;
                }
            }
        ...
        ofs << lastVertexId << "." << state << edgeString << vertexId << "." << state
            << " [style=\"invis\"];" << endl;
        ...
        for (tie(ei, ei_end) = edges(graph); ei != ei_end; ++ei) {
            ...
            ofs << v1Id << "." << state << edgeString << v2Id << "." << state
                << " [color=black,fontcolor=black,label=\"" << weight << "\",penwidth=2];" << endl;
        }
...
void printGraph(TopologyGraph topologyGraph) {
    ...
    for (tie(ei, ei_end) = edges(topologyGraph); ei != ei_end; ++ei) {
        ...
        cout << "edge = " << topologyGraph[*ei].id << " between nodes " << v1Id << " and " << v2Id
             << endl;
    }
}
