    ...
    if (par("enable")) {
        ...
        while (destinationEidTokenizer.hasMoreTokens()) {
            ...
            if (destinationEid > this->getParentModule()->getVectorSize()) {
                throw cException("Error: %s",
                                 (string("wrong destinationEid = ") + destinationEidStr).c_str());
            }
        ...
        if ((bundlesNumberVec_.size() != destinationEidVec_.size()) ||
            (bundlesNumberVec_.size() != sizeVec_.size() ||
             (bundlesNumberVec_.size() != startVec_.size()))) {
            string str1 =
                "Error in App Traffic: Node[" + to_string(this->getParentModule()->getIndex()) +
                "]" + ", bundlesNumberVec, destinationEidVec, sizeVec, startVec: Sizes Missmatch!";
...
void App::handleMessage(cMessage *msg) {
    if (msg->getKind() == TRAFFIC_TIMER) {
        ...
        // Bundle properties
        char bundleName[200];
        sprintf(bundleName, "Src:%d,Dst:%d(id:%d)", (int)this->eid_,
                (int)trafficGenMsg->getDestinationEid(), (int)bundle->getId());
