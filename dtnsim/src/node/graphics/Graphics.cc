...
void Graphics::initialize() {
    ...
    if (hasGUI()) {
        ...
        nodeModule->getParentModule()->getDisplayString().setTagArg("bgb", 0,
                                                                    2 * marginX + 2 * posRadius);
        nodeModule->getParentModule()->getDisplayString().setTagArg("bgb", 1,
                                                                    4 * marginY + 2 * posRadius);
...
void Graphics::setContactOn(ContactMsg *contactMsg) {
    if (hasGUI() && this->par("enable")) {
        ...
        float endPosX =
            marginX + posRadius * cos((contactMsg->getDestinationEid() - 1) * posAngle) + posRadius;
        float endPosY =
            marginY + posRadius * sin((contactMsg->getDestinationEid() - 1) * posAngle) + posRadius;
