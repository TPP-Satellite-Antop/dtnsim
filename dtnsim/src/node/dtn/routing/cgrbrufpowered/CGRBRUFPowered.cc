...
CGRBRUFPowered::CGRBRUFPowered(int eid, SdrModel *sdr, ContactPlan *contactPlan, bool printDebug,
                               ...
                               std::vector<int> ts_start_times, int numOfNodes, string pathPrefix,
                               string pathPosfix)
    ...
    if ((ts_duration != -1 && ts_start_times.size()) ||
        (ts_duration == -1 && ts_start_times.size() == 0)) {
        cout << "[Error] in CGRBRUFPowered::CGRBRUFPowered() : When CGR_BRUFPowered is used you "
                "must set ts_duration or ts_start_times but not both."
             << endl;
        exit(1);
    }
    ...
    for (int source = 1; source <= numOfNodes; source++) {
        for (int target = 1; target <= numOfNodes; target++) {
            if (source != target) {
                ...
                cout << "CGRBrufPowered::CGRBrufPowered check if exist file: " << pathToRouting
                     << endl;
                ...
                if (infile.good()) {
                    ...
                    cout << "CGRBrufPowered::CGRBrufPowered there is not traffic from " << source
                         << " to " << target << endl;
                    // exit(1);
                }
            }
...
void CGRBRUFPowered::cgrForward(BundlePkt *bundle, double simTime) {
    cout << "TIME: " << simTime << "s, NODE: " << eid_
         << ", routing bundle to dst: " << bundle->getDestinationEid() << " ("
    ...
    for (vector<ProximateNode>::iterator it = proximateNodes.begin(); it != proximateNodes.end();
         ++it) {
        cout << "routeTable[" << bundle->getDestinationEid() << "][" << (*it).neighborNodeNbr
             << "]: nextHop: " << (*it).neighborNodeNbr << " (cId:" << (*it).contactId
             << ", resCap:" << contactPlan_->getContactById((*it).contactId)->getResidualVolume()
             << "Bytes) arrivalConf:" << (*it).confidence << " arrivalT:" << (*it).arrivalTime
             << " hopCnt:" << (*it).hopCount << " forfT:" << (*it).forfeitTime << endl;
        // cout << "routeTable[" << terminusNode << "][" << (*it).neighborNodeNbr << "]: nextHop: "
        // << (*it).neighborNodeNbr << ", frm " << route.fromTime << " to " << (*it).arrivalTime <<
        // ", arrival time: " << route.arrivalTime << ", volume: " << route.residualVolume << "/" <<
        ...
        if (bundle->getDlvConfidence() > 0.0 && bundle->getDlvConfidence() < 1.0) {
            float newDlvConfidence =
                1.0 - (1.0 - bundle->getDlvConfidence()) * (1.0 - it->confidence);
    ...
    if (selectedNeighbor != NULL) {
        ...
        cout << "Best: routeTable[" << bundle->getDestinationEid() << "]["
             << selectedNeighbor->neighborNodeNbr
             << "]: nextHop: " << selectedNeighbor->neighborNodeNbr
             << " (cId:" << selectedNeighbor->contactId << ", resCap:"
             << contactPlan_->getContactById(selectedNeighbor->contactId)->getResidualVolume()
             << "Bytes) arrivalConf:" << selectedNeighbor->confidence
             << " arrivalT:" << selectedNeighbor->arrivalTime
             << " hopCnt:" << selectedNeighbor->hopCount
             << " forfT:" << selectedNeighbor->forfeitTime << endl;
    ...
    if (selectedNeighbor !=
        NULL) { // if bundle was enqueued (bundle ->ductXmitElt)
        ...
        cout << "  no chosen neighbor and delivery confidence not reached, enqueueing to limbo"
             << endl;
...
void CGRBRUFPowered::identifyProximateNodes(BundlePkt *bundle, double simTime,
                                            vector<int> excludedNodes,
                                            vector<ProximateNode> *proximateNodes) {
    ...
    if (routeList_[terminusNode].empty() == true) {
        // cout << "    routeList to Node:" << terminusNode << " empty, calling loadRouteList" <<
        // endl;
        loadRouteList(terminusNode, simTime);
        routeListLastEditTime = simTime;
    }

    // cout << "    routeList to Node:" << terminusNode << " size:" <<
    // routeList_[terminusNode].size() << endl;

    vector<CgrRoute>::iterator it = routeList_[terminusNode].begin();
    while (it != routeList_[terminusNode].end()) {
        tableEntriesExplored++;

        cout << "*route through node:" << (*it).nextHop << ", arrivalConf:" << (*it).confidence
             << ", arrivalT:" << (*it).arrivalTime << ", txWin:(" << (*it).fromTime << "-"
        ...
        if ((*it).toTime <= simTime) {
            ...
            // cout << " ignoring, route due, recompute route for contact (not implemented yet!)" <<
            // endl; recomputeRouteForContact();
        ...
        vector<int>::iterator itExl =
            find(excludedNodes.begin(), excludedNodes.end(), (*it).nextHop);
...
void CGRBRUFPowered::tryRoute(BundlePkt *bundle, CgrRoute *route,
                              vector<ProximateNode> *proximateNodes) {
    ...
    for (vector<ProximateNode>::iterator it = (*proximateNodes).begin();
         it != (*proximateNodes).end(); ++it) {
        if ((*it).neighborNodeNbr == route->nextHop) {
            ...
            if (route->confidence > (*it).confidence) {
                // cout << " good route, replace node " << route->toNodeNbr << " in proxNodes" <<
                // endl;
                ...
                // cout << " route through node " << route->toNodeNbr << " in proxNodes has better
                // arrival confidence" << endl;
                return;
            } else if (get_probability_if_this_route_is_chosed(bundle->getSourceEid(),
                                                               bundle->getDestinationEid(), route) >
                       get_probability_if_this_route_is_chosed(
                           bundle->getSourceEid(), bundle->getDestinationEid(), (*it).route)) {
                // cout << " good route, replace node " << route->toNodeNbr << " in proxNodes" <<
                // endl;
                (*it).arrivalTime = route->arrivalTime;
                (*it).hopCount = route->hops.size();
            ...
            // cout << " route through node " << route->toNodeNbr << " in proxNodes has same
            // metrics" << endl;
            return;
        }
    }
    ...
    node.route = route;
    node.success_probability = get_probability_if_this_route_is_chosed(
        bundle->getSourceEid(), bundle->getDestinationEid(), route);
    ...
    while (1) {
        ...
        if (anchorContact != NULL)
            if (firstContact != anchorContact) {
                // cout << "        ending anchored search in contactId: " << anchorContact->getId()
                // << " (src:" << anchorContact->getSourceEid() << "-dst:" <<
                // anchorContact->getDestinationEid() << ", " << anchorContact->getStart() << "s to
                // " << anchorContact->getEnd() << "s)" << endl; This is endAnchoredSearch()
                // function in ion: it clears the working area
                for (vector<Contact>::iterator it = contactPlan_->getContacts()->begin();
                     it != contactPlan_->getContacts()->end(); ++it) {
                    ((Work *)(*it).work)->arrivalTime = numeric_limits<double>::max();
        ...
        if (route.toTime == firstContact->getEnd()) {
            ...
            // cout << "        starting anchored search in contactId: " << anchorContact->getId()
            // << " (src:" << anchorContact->getSourceEid() << "-dst:" <<
            // anchorContact->getDestinationEid() << ", " << anchorContact->getStart() << "s-" <<
            // anchorContact->getEnd() << "s)" << endl; find the limiting contact in route
        ...
        for (vector<Contact>::iterator it = currentNeighbors.begin(); it != currentNeighbors.end();
            ...
            if (arrivalTime < ((Work *)(*it).work)->arrivalTime) {
                ...
                if ((*it).getDestinationEid() == terminusNode) {
                    ...
                    // Confidence criteria to be removed in post 3.5.0
                    if ((*it).getConfidence() > highestConfidence ||
                        (((*it).getConfidence() == highestConfidence) &&
                         ((Work *)(*it).work)->arrivalTime < earliestFinalArrivalTime)) {
...
void CGRBRUFPowered::enqueueToNeighbor(BundlePkt *bundle, ProximateNode *selectedNeighbor) {
    ...
    float newDlvConfidence =
        1.0 - (1.0 - bundle->getDlvConfidence()) * (1.0 - selectedNeighbor->confidence);
...
void CGRBRUFPowered::bpEnqueue(BundlePkt *bundle, ProximateNode *selectedNeighbor) {
    ...
    if (enqueued) {
        if (selectedNeighbor->contactId != 0) {
            ...
            selectedNeighbor->route->hops[0]->setResidualVolume(
                selectedNeighbor->route->hops[0]->getResidualVolume() - bundle->getByteLength());

            cout << "RVol: routeTable[" << bundle->getDestinationEid() << "]["
                 << selectedNeighbor->neighborNodeNbr
                 << "]: new resCap: (cId:" << selectedNeighbor->contactId << ", resCap:"
                 << contactPlan_->getContactById(selectedNeighbor->contactId)->getResidualVolume()
                 << "Bytes)" << endl;
...
double CGRBRUFPowered::get_node_future_delivery_probability(int source_eid, int target_eid,
                                                            int carrier_eid, int ts) {
...
double CGRBRUFPowered::get_probability_if_this_route_is_chosed(int source_eid, int target_eid,
                                                               CgrRoute *route) {
    ...
    for (vector<Contact *>::iterator it = route->hops.begin(); it != route->hops.end(); it++)
        if (route_initial_ts == get_ts_for_contact(*it))
            ...
            break; // When a contact does not belong to the same time stamp that first then the
                   // following contacts neither.

    int last_hop_destination_in_initial =
        first_ts_contacts.back()->getDestinationEid(); // At which node this bundle will arrive in
                                                       // current timestamp is all go well

    double route_successful_probability =
        pow(1 - this->contact_failure_probability, first_ts_contacts.size()) *
        get_node_future_delivery_probability(source_eid, target_eid,
                                             last_hop_destination_in_initial, route_initial_ts + 1);

    for (unsigned int num_of_working_links = 0; num_of_working_links < first_ts_contacts.size();
         num_of_working_links++)
        route_successful_probability +=
            pow(1 - this->contact_failure_probability, num_of_working_links) *
            this->contact_failure_probability *
            get_node_future_delivery_probability(
                source_eid, target_eid, first_ts_contacts.at(num_of_working_links)->getSourceEid(),
                route_initial_ts + 1);
