...
RoutingCgrModelRev17::RoutingCgrModelRev17(int eid, int nodeNum, SdrModel *sdr,
                                           ContactPlan *localContactPlan,
                                           ContactPlan *globalContactPlan, string routingType,
                                           bool printDebug)
...
void RoutingCgrModelRev17::routeAndQueueBundle(BundlePkt *bundle, double simTime) {
    ...
    cout << "TIME: " << simTime_ << "s, NODE: " << eid_
         << ", routing bundle : " << bundle->getSourceEid() << "-" << bundle->getDestinationEid()
    ...
    if (routingType_.find("extensionBlock:on") != std::string::npos) {
        if (bundle->getCgrRoute().nextHop == EMPTY_ROUTE) {
            ...
            cout << "extensionBlockRoute: [" << ebRoute.terminusNode << "][" << ebRoute.nextHop
                 << "]: nextHop: " << ebRoute.nextHop << ", frm " << ebRoute.fromTime << " to "
                 << ebRoute.toTime << ", arrival time: " << ebRoute.arrivalTime
                 << ", volume: " << ebRoute.residualVolume << "/" << ebRoute.maxVolume << "Bytes"
                 << endl
                 << "hops: ";
            for (vector<Contact *>::iterator hop = ebRoute.hops.begin(); hop != ebRoute.hops.end();
                 ++hop)
                cout << "(+" << (*hop)->getStart() << " +" << (*hop)->getEnd() << " "
                     << (*hop)->getSourceEid() << " " << (*hop)->getDestinationEid() << " "
                     << (*hop)->getResidualVolume() << "/" << (*hop)->getVolume() << "Bytes-local:"
            ...
            if (ebRouteIsValid) {
                // Update necessary route parameters (from/to time and max/residual volume not
                // necessary)
...
void RoutingCgrModelRev17::cgrForward(BundlePkt *bundle) {
    ...
    if (routingType_.find("contactPlan:global") != std::string::npos) {
        ...
        if (!routeTable_.at(terminusNode).empty())
            for (unsigned int r = 0; r < routeTable_.at(terminusNode).size(); r++)
                for (vector<Contact *>::iterator hop1 =
                         routeTable_.at(terminusNode).at(r).hops.begin();
                     hop1 != routeTable_.at(terminusNode).at(r).hops.end(); ++hop1)
                    if (routeTable_.at(terminusNode).at(r).residualVolume >
                        (*hop1)->getResidualVolume()) {
                        routeTable_.at(terminusNode).at(r).residualVolume =
                            (*hop1)->getResidualVolume();
                        cout << "*Rvol: routeTable[" << terminusNode << "][" << r
                             << "]: updated to " << (*hop1)->getResidualVolume()
                             << "Bytes (all contacts)" << endl;
                    }
    }
            ...
            while (1) {
                ...
                if (anchorContact != NULL)
                    ...
                    if (firstContact != anchorContact) {
                        ...
                        cout << "-anchored search ends in contactId: " << anchorContact->getId()
                             << " (src:" << anchorContact->getSourceEid()
                             << "-dst:" << anchorContact->getDestinationEid() << ", "
                             << anchorContact->getStart() << "s to " << anchorContact->getEnd()
                             << "s)" << endl;

                        // Unsupress all remote contacts (suppressed local contacts dont change)
                        for (vector<int>::iterator it = suppressedContactIds.begin();
                             it != suppressedContactIds.end();)
                ...
                cout << "-new route found through node:" << route.nextHop
                ...
                if (route.toTime == firstContact->getEnd())
                    ...
                    cout << "-anchored search starts in contactId: " << anchorContact->getId()
                         << " (src:" << anchorContact->getSourceEid()
                         << "-dst:" << anchorContact->getDestinationEid() << ", "
                         << anchorContact->getStart() << "s-" << anchorContact->getEnd() << "s)"
                         << endl;

                    // Find the limiting contact in route
                    for (vector<Contact *>::iterator it = route.hops.begin();
                         it != route.hops.end(); ++it)
                ...
                cout << "-suppressing limiting contactId: " << limitContact->getId()
                     << " (src:" << limitContact->getSourceEid()
                     << "-dst:" << limitContact->getDestinationEid() << ", "
    ...
    if (routingType_.find("routeListType:oneBestPath") != std::string::npos) {
        ...
        if (routeTable_.at(terminusNode).at(0).residualVolume < bundle->getByteLength()) {
            ...
            for (hop = routeTable_.at(terminusNode).at(0).hops.begin();
                ...
                if (routeTable_.at(terminusNode).at(0).residualVolume ==
                    (*hop)->getResidualVolume())
        ...
        if (needRecalculation) {
            ...
            if (routeTable_.at(terminusNode).at(1).residualVolume < bundle->getByteLength()) {
                ...
                for (hop = routeTable_.at(terminusNode).at(1).hops.begin();
                    ...
                    if (routeTable_.at(terminusNode).at(1).residualVolume ==
                        (*hop)->getResidualVolume())
                        (*hop)->setResidualVolume(0);
    ...
    if (!routeTable_.at(terminusNode).empty()) {
        ...
        bestRoute = min_element(routeTable_.at(terminusNode).begin(),
                                routeTable_.at(terminusNode).end(), this->compareRoutes);
...
void RoutingCgrModelRev17::cgrEnqueue(BundlePkt *bundle, CgrRoute *bestRoute) {
    ...
    if (bestRoute->nextHop != NO_ROUTE_FOUND && !bestRoute->filtered) {
        ...
        cout << "*Best: routeTable[" << bestRoute->terminusNode
             << "][ ]: nextHop: " << bestRoute->nextHop << ", frm " << bestRoute->fromTime << " to "
             << bestRoute->toTime << ", arrival time: " << bestRoute->arrivalTime
             << ", volume: " << bestRoute->residualVolume << "/" << bestRoute->maxVolume << "Bytes"
             << endl;
        ...
        if (routingType_.find("contactPlan:local") != std::string::npos ||
            ...
            if (routingType_.find("volumeAware:allContacts") != std::string::npos) {
                ...
                for (vector<Contact *>::iterator hop = bestRoute->hops.begin();
                     hop != bestRoute->hops.end(); ++hop) {
                    (*hop)->setResidualVolume((*hop)->getResidualVolume() -
                                              bundle->getByteLength());
                ...
                for (int n = 1; n < nodeNum_; n++)
                    if (!routeTable_.at(n).empty())
                        for (unsigned int r = 0; r < routeTable_.at(n).size(); r++)
                            for (vector<Contact *>::iterator hop1 =
                                     routeTable_.at(n).at(r).hops.begin();
                                ...
                                for (vector<Contact *>::iterator hop2 = bestRoute->hops.begin();
                                    ...
                                    if ((*hop1)->getId() == (*hop2)->getId())
                                        // Does the reduction of this contact volume requires a
                                        // route volume update?
                                        if (routeTable_.at(n).at(r).residualVolume >
                                            (*hop1)->getResidualVolume()) {
                                            routeTable_.at(n).at(r).residualVolume =
                                                (*hop1)->getResidualVolume();
                                            cout << "*Rvol: routeTable[" << n << "][" << r
                                                 << "]: updated to " << (*hop1)->getResidualVolume()
                                                 << "Bytes (all contacts)" << endl;
                                        }
            }
            ...
            if (routingType_.find("volumeAware:1stContact") != std::string::npos) {
                ...
                bestRoute->hops[0]->setResidualVolume(bestRoute->hops[0]->getResidualVolume() -
                                                      bundle->getByteLength());
...
void RoutingCgrModelRev17::findNextBestRoute(vector<int> suppressedContactIds, int terminusNode,
                                             CgrRoute *route) {
    ...
    for (auto &contact : *contactPlan_->getContacts()) {
        ...
        auto it =
            std::find(suppressedContactIds.begin(), suppressedContactIds.end(), contact.getId());
    ...
    priority_queue<pair<double, int>, vector<pair<double, int>>, std::greater<pair<double, int>>>
        pq;
    ...
    while (!pq.empty() && pq.top().second != terminusNode) {
        ...
        for (auto &edgeId : *contactPlan_->getContactIdsBySrc(currentNode)) {
            ...
            if (((Work *)edgeToNeighbor->work)->suppressed ||
                edgeToNeighbor->getEnd() <= arrivalTime)
                continue;
    ...
    if (predecesors[terminusNode] != NULL) {
        ...
        route->hops.clear();
        for (Contact *contact = predecesors[terminusNode]; contact != NULL;
             contact = predecesors[contact->getSourceEid()]) {
        ...
        for (Contact *contact : route->hops) {
            route->toTime = std::min(route->toTime, contact->getEnd() - accumulatedRange);
...
void RoutingCgrModelRev17::printRouteTable(int terminusNode) {
    ...
    for (unsigned int r = 0; r < routeTable_.at(terminusNode).size(); r++) {
        ...
        if (route.nextHop == NO_ROUTE_FOUND)
            ...
            cout << "routeTable[" << terminusNode << "][" << r << "]: Need to recalculate route"
                 << endl;
        else {
            cout << "routeTable[" << terminusNode << "][" << r << "]: nextHop: " << route.nextHop
                 << ", frm " << route.fromTime << " to " << route.toTime
                 << ", arrival time: " << route.arrivalTime << ", volume: " << route.residualVolume
                 << "/" << route.maxVolume << "Bytes: ";

            // print route:
            for (vector<Contact *>::iterator hop = route.hops.begin(); hop != route.hops.end();
                 ++hop)
                cout << "(+" << (*hop)->getStart() << "+" << (*hop)->getEnd() << " "
                     << (*hop)->getSourceEid() << " " << (*hop)->getDestinationEid() << " "
                     << (*hop)->getResidualVolume() << "/" << (*hop)->getVolume() << "Bytes)"
                     << ", toTime: " << route.toTime;
...
void RoutingCgrModelRev17::checkRoutingTypeString() {
    ...
    if (routingType_.compare("none") == 0) {
        routingType_ = "contactPlan:local,routeListType:perNeighborBestPath,volumeAware:"
                       "allContacts,extensionBlock:off";
    ...
    if (routingType_.find("contactPlan:local") == std::string::npos &&
        routingType_.find("contactPlan:global") == std::string::npos) {
        cout << "dtnsim error: unknown or missing contactPlan type in routingType string: "
             << routingType_ << endl;
        exit(1);
    }
    ...
    if (routingType_.find("routeListType:allPaths-yen") == std::string::npos &&
        ...
        cout << "dtnsim error: unknown or missing routeListType type in routingType string: "
             << routingType_ << endl;
        exit(1);
    }
    ...
    if (routingType_.find("volumeAware:off") == std::string::npos &&
        ...
        cout << "dtnsim error: unknown or missing volumeAware type in routingType string: "
             << routingType_ << endl;
        exit(1);
    }
    ...
    if (routingType_.find("extensionBlock:on") == std::string::npos &&
        ...
        cout << "dtnsim error: unknown or missing extensionBlock type in routingType string: "
             << routingType_ << endl;
        exit(1);
    }
