...
void RoutingCgrModel350_2Copies::routeAndQueueBundle(BundlePkt *bundle, double simTime) {
    ...
    if (bundle->getQos() == 0 || bundle->getQos() == 1) {
        ...
        selectedNeighbor =
            cgrForward(bundle, simTime, (bundle->getQos() == 0) ? arrivalTime : hopCount);
        ...
        if (selectedNeighborH == NULL) {
            cout << "RoutingCGRModel350_Proactive::routeAndQueueBundle(BundlePkt * bundle, double "
                    "simTime) - Route by arrival time exist but no by hops!!."
                 << endl;
            exit(0);
        }
...
RoutingCgrModel350_2Copies::ProximateNode *
RoutingCgrModel350_2Copies::cgrForward(BundlePkt *bundle, double simTime, Mode mode) {
...
void RoutingCgrModel350_2Copies::identifyProximateNodes(BundlePkt *bundle, double simTime,
                                                        vector<int> excludedNodes,
                                                        vector<ProximateNode> *proximateNodes,
                                                        Mode mode) {
...
void RoutingCgrModel350_2Copies::tryRoute(BundlePkt *bundle, CgrRoute *route,
...
void RoutingCgrModel350_2Copies::findNextBestRoute(Contact *rootContact, int terminusNode,
                                                   CgrRoute *route) {
    // increment counter
    dijkstraCalls++;
...
void RoutingCgrModel350_2Copies::enqueueToNeighbor(BundlePkt *bundle,
                                                   ProximateNode *selectedNeighbor) {
 ...
 */
void RoutingCgrModel350_2Copies::successfulBundleForwarded(long bundleId, Contact *contact,
