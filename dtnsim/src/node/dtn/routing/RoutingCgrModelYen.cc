...
void RoutingCgrModelYen::cgrForward(BundlePkt *bundle, double simTime) {
    cout << endl
         << "simTime: " << simTime << "s, Node " << eid_
         << ", calling cgrForward (src: " << bundle->getSourceEid()
         << ", dst:" << bundle->getDestinationEid()
         << ", created:" << bundle->getCreationTimestamp().dbl() << "s, id:" << bundle->getId()
         << ")" << endl;
    ...
    for (vector<ProximateNode>::iterator it = proximateNodes.begin(); it != proximateNodes.end();
         ++it) {
        cout << "    ProxNode " << (*it).neighborNodeNbr << " (cId:" << (*it).contactId
             << ") arrivalConf:" << (*it).arrivalConfidence << " arrivalT:" << (*it).arrivalTime
             << " hopCnt:" << (*it).hopCount << " forfT:" << (*it).forfeitTime << ":";
        ...
        if (bundle->getDlvConfidence() > 0.0 && bundle->getDlvConfidence() < 1.0) {
            float newDlvConfidence =
                1.0 - (1.0 - bundle->getDlvConfidence()) * (1.0 - it->arrivalConfidence);
...
void RoutingCgrModelYen::identifyProximateNodes(BundlePkt *bundle, double simTime,
                                                vector<int> excludedNodes,
                                                vector<ProximateNode> *proximateNodes) {
    ...
    cout << "    routeList to Node:" << terminusNode << " size:" << routeList_[terminusNode].size()
         << endl;

    for (vector<CgrRoute>::iterator it = routeList_[terminusNode].begin();
         it != routeList_[terminusNode].end(); ++it) {
        cout << "      route through node:" << (*it).toNodeNbr
             << ", arrivalConf:" << (*it).arrivalConfidence << ", arrivalT:" << (*it).arrivalTime
             << ", txWin:(" << (*it).fromTime << "-" << (*it).toTime
             << "), maxCap:" << (*it).maxCapacity << "bits:";
        ...
        vector<int>::iterator itExl =
            find(excludedNodes.begin(), excludedNodes.end(), (*it).toNodeNbr);
...
void RoutingCgrModelYen::tryRoute(BundlePkt *bundle, CgrRoute *route,
                                  vector<ProximateNode> *proximateNodes) {
    ...
    for (vector<ProximateNode>::iterator it = (*proximateNodes).begin();
        ...
        if ((*it).neighborNodeNbr == route->toNodeNbr) {
            ...
            if (route->arrivalConfidence > (*it).arrivalConfidence) {
                ...
                cout << " route through node " << route->toNodeNbr
                     << " in proxNodes has better arrival confidence" << endl;
                return;
                ...
                cout << " route through node " << route->toNodeNbr
                     << " in proxNodes has better arrival time" << endl;
                return;
                ...
                cout << " route through node " << route->toNodeNbr
                     << " in proxNodes has better hop count" << endl;
                return;
            ...
            cout << " route through node " << route->toNodeNbr << " in proxNodes has same metrics"
                 << endl;
            return;
        }
    }
...
void RoutingCgrModelYen::loadRouteList(int terminusNode, double simTime) {
    ...
    while (1) {
        ...
        // Record route
        cout << "      new route found through node:" << route.toNodeNbr
             << ", arrivalConf:" << route.arrivalConfidence << ", arrivalT:" << route.arrivalTime
             << ", txWin:(" << route.fromTime << "-" << route.toTime
             << "), maxCap:" << route.maxCapacity << "bits:" << endl;
        ...
        cout << "        supressing limiting contactId: " << limitContact->getId()
             << " (src:" << limitContact->getSourceEid()
             << "-dst:" << limitContact->getDestinationEid() << ", " << limitContact->getStart()
             << "s-" << limitContact->getEnd() << "s)" << endl;
...
void RoutingCgrModelYen::loadRouteListYen(int terminusNode, double simTime) {
    ...
    while (1) { // for k from 1 to K:
        ...
        for (; it != --routeList_[terminusNode].back().hops.end(); ++it) {
            ...
            cout << "          spur node " << (*it)->getId() << " (+" << (*it)->getStart() << " +"
                 << (*it)->getEnd() << " " << (*it)->getSourceEid() << " "
                 << (*it)->getDestinationEid() << ")" << endl;

            // The sequence of nodes from the source to the spur node of the previous k-shortest
            // path. If no sorting happens, the working area will remain with the arrivalTime
            // metrics of each contact which we can use for the rootPath metrics. At this point we
            // should save necesary metrics.
            CgrRoute rootPath;
            ...
            // rootPath.toNodeNbr = routeList_[terminusNode].back().toNodeNbr; we might not know
            // which is the toNodeNbr
            rootPath.arrivalTime = arrivalTime;
            rootPath.fromTime = routeList_[terminusNode].back().fromTime;
            cout << " toNodeNbr: TBD, arrTime:" << rootPath.arrivalTime
                 << ", fromTime:" << rootPath.fromTime << ", toTime:" << rootPath.toTime
                 << ", maxCap:" << rootPath.maxCapacity << endl;

            // Add back the edges and nodes that were removed from the graph
            // Clear working area
            for (vector<Contact>::iterator it2 = contactPlan_->getContacts()->begin();
                 it2 != contactPlan_->getContacts()->end(); ++it2) {
            ...
            // root path.
            for (vector<CgrRoute>::iterator it2 = routeList_[terminusNode].begin();
                 it2 != routeList_[terminusNode].end(); ++it2) {
                ...
                for (it3 = rootPath.hops.begin(), it4 = (*it2).hops.begin();
                     (it3 != rootPath.hops.end()) && (it4 != (*it2).hops.end()); ++it3, ++it4) {
                ...
                nextContact = (*it4); // there is allways a next contact because it gets to the
                                      // penultimate node in the path.
            ...
            for (vector<Contact *>::reverse_iterator it2 = rootPath.hops.rbegin();
                 it2 != rootPath.hops.rend(); ++it2) {
        ...
        if (routeContainerB.empty()) {
            cout << "        container B is empty, ending Yen's with "
                 << routeList_[terminusNode].size() << " routes in route list: ";
...
void RoutingCgrModelYen::findNextBestRoute(Contact *rootContact, int terminusNode,
                                           CgrRoute *route) {
...
void RoutingCgrModelYen::enqueueToNeighbor(BundlePkt *bundle, ProximateNode *selectedNeighbor) {
    ...
    float newDlvConfidence =
        1.0 - (1.0 - bundle->getDlvConfidence()) * (1.0 - selectedNeighbor->arrivalConfidence);
