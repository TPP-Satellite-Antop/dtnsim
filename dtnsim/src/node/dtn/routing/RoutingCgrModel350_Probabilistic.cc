...
void RoutingCgrModel350_Probabilistic::routeAndQueueBundle(BundlePkt *bundle, double simTime) {
    ...
    if (bundle->getQos() == 0) {
        ...
        if (selectedNeighborH == NULL) {
            cout << "RoutingCGRModel350_Probabilistic::routeAndQueueBundle(BundlePkt * bundle, "
                    "double simTime) - Route by arrival time exist but no by hops!!."
                 << endl;
            exit(0);
        }
        ...
        if (selectedNeighborAT->neighborNodeNbr == selectedNeighborH->neighborNodeNbr) {
            cout << "Best: routeTable[" << bundle->getDestinationEid() << "]["
                 << selectedNeighborAT->neighborNodeNbr
                 << "]: nextHop: " << selectedNeighborAT->neighborNodeNbr
                 << " (cId:" << selectedNeighborAT->contactId << ", resCap:"
                 << contactPlan_->getContactById(selectedNeighborAT->contactId)->getResidualVolume()
                 << "Bytes) arrivalConf:" << selectedNeighborAT->confidence
                 << " arrivalT:" << selectedNeighborAT->arrivalTime
                 << " hopCnt:" << selectedNeighborAT->hopCount
                 << " forfT:" << selectedNeighborAT->forfeitTime << endl;
            ...
            // Send bundle using best arrival time route if the route success probability is greater
            // or equal than 0.5
            if (pow(sContactProb_, selectedNeighborAT->hopCount) >= 0.5) {
                ...
                cout << "Best: routeTable[" << bundle->getDestinationEid() << "]["
                     ...
                     << "]: nextHop: " << selectedNeighborAT->neighborNodeNbr
                     << " (cId:" << selectedNeighborAT->contactId << ", resCap:"
                     << contactPlan_->getContactById(selectedNeighborAT->contactId)
                            ->getResidualVolume()
                     << "Bytes) arrivalConf:" << selectedNeighborAT->confidence
                     << " arrivalT:" << selectedNeighborAT->arrivalTime
                     << " hopCnt:" << selectedNeighborAT->hopCount
            ...
            bundleCopy->setQos(1);
            cout << "Best: routeTable[" << bundle->getDestinationEid() << "]["
                 << selectedNeighborH->neighborNodeNbr
                 << "]: nextHop: " << selectedNeighborH->neighborNodeNbr
                 << " (cId:" << selectedNeighborH->contactId << ", resCap:"
                 << contactPlan_->getContactById(selectedNeighborH->contactId)->getResidualVolume()
                 << "Bytes) arrivalConf:" << selectedNeighborH->confidence
                 << " arrivalT:" << selectedNeighborH->arrivalTime
                 << " hopCnt:" << selectedNeighborH->hopCount
                 << " forfT:" << selectedNeighborH->forfeitTime << endl;
        ...
        cout << "RoutingCGRModel350_Probabilistic::routeAndQueueBundle(BundlePkt * bundle, double "
                "simTime) - Bundle QoS parameter has invalid value."
             << endl;
        exit(0);
    }
...
RoutingCgrModel350_Probabilistic::ProximateNode *
RoutingCgrModel350_Probabilistic::cgrForward(BundlePkt *bundle, double simTime, Mode mode) {
...
void RoutingCgrModel350_Probabilistic::identifyProximateNodes(BundlePkt *bundle, double simTime,
                                                              vector<int> excludedNodes,
                                                              vector<ProximateNode> *proximateNodes,
                                                              Mode mode) {
...
void RoutingCgrModel350_Probabilistic::tryRoute(BundlePkt *bundle, CgrRoute *route,
                                                vector<ProximateNode> *proximateNodes, Mode mode) {
        ...
        for (vector<ProximateNode>::iterator it = (*proximateNodes).begin();
            ...
            if ((*it).neighborNodeNbr == route->nextHop) {
                ...
                if (route->confidence > (*it).confidence) {
                    ...
                    // cout << " route through node " << route->toNodeNbr << " in proxNodes has
                    // better arrival confidence" << endl;
                    return;
...
void RoutingCgrModel350_Probabilistic::findNextBestRoute(Contact *rootContact, int terminusNode,
                                                         CgrRoute *route) {
    // increment counter
    dijkstraCalls++;
...
void RoutingCgrModel350_Probabilistic::enqueueToNeighbor(BundlePkt *bundle,
                                                         ProximateNode *selectedNeighbor) {
 ...
 */
void RoutingCgrModel350_Probabilistic::successfulBundleForwarded(long bundleId, Contact *contact,
