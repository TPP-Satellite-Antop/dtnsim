...
typedef enum { DiscardedRoute = 0, CandidateRoute = 1 } PhaseTwoRouteFlag;
 ...
 * \return PhaseTwoSAP*
 *
 * \retval  PhaseTwoSAP*     The struct with all the values used by phase two during the current
 *call
 *
 ...
 *
 * \param[in]	target               Is this ipn node number included into the excludedNeighbors
 *list? \param[in]	excludedNeighbors    The list of excluded neighbors
 *
 ...
 *
 * \param[in]    neighborsFound     The number of neighbors that currently are associated with at
 *least one candidate route. \param[in]    suppressedFound    The number of suppressed neighbors
 *(e.g. excluded, or closing loop with a candidate route). \param[in]    neighborsLimit     The
 *total number of neighbors for which we want a candidate route, including those already found.
 * \param[in]    maxNeighborsNumber The total number of neighbors for which we could find a route to
 *destination
 *
 *
 * \par Revision History:
 ...
 *****************************************************************************/
int reached_neighbors_limit(long unsigned int neighborsFound, long unsigned int suppressedFound,
                            long unsigned int neighborsLimit,
                            long unsigned int maxNeighborsNumber) {
    int result = 0;
 ...
 *                                         at the end it will contains the amount of bytes that will
 *not be reforwarded by the overbooking management \param[out]       *allotment            Bytes
 *that could be overbooked \param[out]       *volume               Applicable contact volume of the
 *first contact of the route, computed as the product of the xmitRate and the applicable duration
 ...
 * 			2.  In this function the route->protected field will be updated (overbooking
 *management).
 *
 *
 * \par Revision History:
 ...
 *  06/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int computeResidualBacklog(unsigned long regionNbr, time_t current_time,
                                  unsigned long long localNode, Route *route, CgrScalar *allotment,
                                  CgrScalar *volume, CgrScalar *residualBacklog) {
    int result = -1;
 ...
 *                      the lower stop time between all contacts from this contact to the last
 *contact of the hops list
 *
 * \param[in]		*contact                A contact of the hops list.
 * \param[in]		priority                The priority level of the bundle
 * \param[in]		firstByteTransmitTime   The first byte transmit time for the sender node of
 *the contact
 * \param[in]		*elt                    The ListElt for which the contact is the data field
 * \param[out]		*effectiveVolumeLimit   The effective volume limit computed, only in success
 *case
 *
...
static int computeEffectiveVolumeLimit(UniboContact *contact, Priority priority,
                                       time_t firstByteTransmitTime, ListElt *elt,
    ...
    if (effectiveDuration <= 0) {
        ...
        *effectiveVolumeLimit = (double)((long unsigned int)effectiveDuration * contact->xmitRate);
 ...
 * \retval      -1      The route isn't viable for the forwarding of the bundle, PBAT hasn't been
 *computed
 *
 * \param[in]   current_time           The time used during the current call
...
static int computeExpectedBundleDeliveryTime(time_t current_time, CgrBundle *bundle, Route *route,
    ...
    *lastByteArrivalTime = 0;
    ...
    if (contact->xmitRate > 0) {
        ...
        firstByteTransmitTime = startTime + ((ONE_GIG * applicableRadiationLatency.gigs) +
        ...
        divideCgrScalar(&applicableRadiationLatency, (long int)contact->xmitRate);
        lastByteTransmitTime =
            firstByteTransmitTime + ((ONE_GIG * applicableRadiationLatency.gigs) +
                                     applicableRadiationLatency.units); // SABR 3.2.6.4
                ...
                *lastByteArrivalTime = lastByteTransmitTime + owlt + owltMargin; // SABR 3.2.6.6
                ...
                if (contact->mtv[priority] <= 0.0) // SABR 3.2.6.8.11
                {
                    ...
                    } else if (effectiveVolumeLimit < bundle->evc) // TODO do not fragment
                    {
                        ...
                        // SABR 3.2.6.8.10: RVL
                        route->routeVolumeLimit = (route->routeVolumeLimit < effectiveVolumeLimit)
                        ...
                        if (contact != NULL) {
                            ...
                            nominalContactVolume =
                                (double)((long unsigned int)(contact->toTime - contact->fromTime) *
                                         contact->xmitRate);
                            ...
                            loadCgrScalar(&applicableRadiationLatency, (long int)bundle->evc);
                            divideCgrScalar(&applicableRadiationLatency,
                                            (long int)contact->xmitRate);
 ...
 *
 * \par Notes:
 * 			1.	At the end, in success case, the following route's fields will be
 *updated: -	eto -	pbat -	routeVolumeLimit -	owltSum -	arrivalTime -	bundleEVC -
 ...
 *  06/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int computePBAT(time_t current_time, unsigned long long localNode, Route *route,
                       CgrBundle *bundle) {
    int result = -1;
    CgrScalar applicableBacklog, totalBacklog, allotment, volume, residualBacklog;
    time_t lastByteArrivalTime;

    if (computeApplicableBacklog(route->neighbor, bundle->priority_level, bundle->ordinal,
        ...
        if (computeResidualBacklog(bundle->regionNbr, current_time, localNode, route, &allotment,
                                   &volume, &residualBacklog) < 0) {
            result = -3;
        } else {
            ...
            result = computeExpectedBundleDeliveryTime(current_time, bundle, route,
                                                       &residualBacklog, &lastByteArrivalTime);
 ...
 * \retval   0  Phase one will discard this route, so don't add it into the subset (OR neighbor not
 *found) \retval  -2  MWITHDRAW error
 *
 ...
 *  30/04/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int suppress_destination_excluded_neighbors(Node *destination, List suppressedNeighbors,
...
#if (CGR_AVOID_LOOP > 0)
        if (newCandidateRoute->checkValue == FAILED_NEIGHBOR ||
            newCandidateRoute->checkValue == CLOSING_LOOP) {
            ...
            if (!(CANDIDATE_ROUTES_FOUND(neighbor))) {
                if (suppress_neighbor(sap->suppressedNeighbors, &(newCandidateRoute->neighbor)) ==
                    -2) {
                    result = -2;
                }
            }
 ...
 * \return int
 *
 * \retval   1   Success case: nothing to do, we have already found a candidate route for N
 *neighbors \retval   0   Success case \retval  -1   route's neighbor not found \retval  -2
 *MWITHDRAW error
 *
 * \param[in]  routeFlag                  Flag used to know if route is a candidate or discarded
 *route \param[in]  *route                     Generally a route, it could be a candidate route or
 *not \param[in]  neighbors_limit            We want a candidate route for N neighbors, where N is
 *the limit. This is the initial value of missing neighbors (N), so the real number of missing
 *neighbors is done by the result of "neighbors_limit - neighborsFound " (if greater than 0, obv).
 ...
 *  30/04/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int phase_one_conversation_management(PhaseTwoRouteFlag routeFlag, Route *route,
                                             long unsigned int neighbors_limit,
                                             long unsigned int max_neighbors_number) {
    Neighbor *neighbor;
    int result = 0;
    PhaseTwoSAP *sap = get_phase_two_sap(NULL);

    if (reached_neighbors_limit(sap->neighborsFound, sap->suppressedNeighbors->length,
                                neighbors_limit, max_neighbors_number)) {
...
static int loopIdentified(Route *route, CgrBundle *bundle) {
    ...
    if (result == 0 && route->hops != NULL) {
        ...
        for (elt = route->hops->first; elt != last_contact && elt != NULL && !found;
             elt = elt->next) {
 ...
 * \retval   -4  Route not viable: Best-case delivery time is after than the bundle's deadline
 *SABR 3.2.6.9 a) \retval   -5  Route not viable: (no SABR) The first contact has a confidence less
 *than 1.0 (used in ION) \retval   -6  Route not viable: (no SABR) The route adds to the bundle's
 *delivery confidence a not considerable confidence (used in ION) \retval   -7  Route not viable:
 *The neighbor is the local node but the local node isn't the bundle's destination node SABR 3.2.6.9
 *c) \retval   -8  Route not viable: The neighbor is in the excluded nodes list SABR 3.2.6.9 b)
 ...
 *  06/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
int checkRoute(time_t current_time, unsigned long long localNode, CgrBundle *bundle,
...
#if (NEGLECT_CONFIDENCE == 0)
        else if (firstContact->confidence <
                 1.0F) //(no SABR) first contact must be certain (confidence == 1.0F)
        {
            result = -5;
...
#endif
        else if (route->neighbor == localNode &&
                 bundle->terminus_node != localNode) // SABR 3.2.6.9 c)
        {
            result = -7;
 ...
 *****************************************************************************/
int getCandidateRoutes(Node *terminusNode, CgrBundle *bundle, List excludedNeighbors,
    ...
    if (terminusNode != NULL && terminusNode->routingObject != NULL &&
        ...
        excludedNeighbors != NULL && computedRoutes != NULL && subsetComputedRoutes != NULL &&
        missingNeighbors != NULL && candidateRoutes != NULL) {
        ...
        *missingNeighbors = get_local_node_neighbors_count();
        ...
        if (max_neighbors_number != sap->last_max_neighbors_number) {
            //			debug_printf("Suppressing excluded nodes (last: %lu, current:
            //%lu)...",last_max_neighbors_number, max_neighbors_number);
            if (suppress_destination_excluded_neighbors(terminusNode, sap->suppressedNeighbors,
                                                        excludedNeighbors) < 0) {
                result = -2;
                error = 1;
            }
        ...
        for (elt = computedRoutes->first; elt != NULL && !error; elt = elt->next) {
            if (elt->data != NULL) {
                ...
                if (check == 0) {
                    ...
                    if (phase_one_conversation_management(CandidateRoute, route, *missingNeighbors,
                ...
                } else if (check == -3 || check == -4 || check == -5 || check == -6 ||
                           check == -9 || (check == -8 && MAX_DIJKSTRA_ROUTES == 1)) {
                    // Ok, this route isn't viable
                    // but the checkRoute() said that
                    // this route could be used as "fromRoute" to compute other routes

                    if (phase_one_conversation_management(DiscardedRoute, route, *missingNeighbors,
                                                          max_neighbors_number) < 0) {
                        result = -2;
                        error = 1;
                    }
        ...
        if (result < 0) {
            ...
            *candidateRoutes = (sap->routes->length > 0) ? sap->routes : NULL;
            ...
            if (reached_neighbors_limit(sap->neighborsFound, sap->suppressedNeighbors->length,
                                        *missingNeighbors, max_neighbors_number)) {
                *missingNeighbors = 0;
                *subsetComputedRoutes = NULL;
        ...
        debug_printf("%lu neighbors found, %lu missing neighbors, %lu suppressed neighbors.",
                     sap->neighborsFound, *missingNeighbors, sap->suppressedNeighbors->length);
    }
