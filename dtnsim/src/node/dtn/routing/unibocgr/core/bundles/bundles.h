...
typedef enum { Bulk = 0, Normal, Expedited } Priority;
...
typedef struct {
     ...
     * - Backward propagation bit (SET_BACKWARD_PROPAGATION,UNSET_BACKWARD_PROPAGATION and
     * RETURN_TO_SENDER macros)
...
extern int initialize_bundle(int backward_propagation, int critical, float dlvConfidence,
                             time_t expiration_time, Priority priority, unsigned int ordinal,
                             int probe, int fragmentable, long unsigned int size,
                             unsigned long long sender_node, unsigned long long terminus_node,
                             CgrBundle *bundle);
...
#define print_log_bundle_id(source, timestamp, seq_number, fragm_length, fragm_offset)             \
    writeLog("Bundle ID { source: %llu, creation timestamp (msec): %llu, sequence number: %u, "    \
             "fragm. length: %u, fragm. offset: %u }.",                                            \
...
#define print_bundle(file_call, bundle, excludedNodes, current_time)                               \
    do {                                                                                           \
    } while (0)
#endif
