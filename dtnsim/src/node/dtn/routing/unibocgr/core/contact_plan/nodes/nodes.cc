 ...
 *
 * \warning Use this function ONLY when you are deleting all the routes and all the references from
 *the contacts graph.
 *
 * \par Revision History:
 *
 ...
 *
 * \warning Use this function ONLY when you are deleting all the routes and all the references from
 *the contacts graph (for efficiency).
 *
 * \par Notes:
 *              1.  If you call this function you must call even the correspective function for the
 *contacts graph.
 *
 * \par Revision History:
 *
...
Neighbor *get_neighbor(unsigned long long node_number) {
    ...
    if (sap->local_node_neighbors != NULL) {
        for (elt = sap->local_node_neighbors->first; elt != NULL && result == NULL;
             elt = elt->next) {
            if (elt->data != NULL) {
 ...
 *                          Each data in this list is an unsigned long long (the ipn number of the
 *neighbor) \param[in]  destination  The destination node where we store the citations to neighbors
...
int insert_neighbors_to_reach_destination(List neighbors, Node *destination) {
    ...
    if (sap->local_node_neighbors == NULL) {
        ...
        if (NEIGHBORS_DISCOVERED(rtgObj) && (neighbors->length == rtgObj->citations->length)) {
            ...
            debug_printf("Discovered new total neighbors number (%lu) to reach destination %llu. "
                         "Previous total number (%lu)",
                         neighbors->length, destination->nodeNbr, rtgObj->citations->length);
...
static int add_neighbor(unsigned long long node_number, time_t to_time) {
    ...
    if (neighbor != NULL) {
        ...
        if (neighbor != NULL) {
            ...
            verbose_debug_printf("Can't create neighbor %llu (toTime: %ld)", node_number,
                                 (long int)to_time);
        }
    }
    ...
    if (!sap->neighbors_list_builded) {
        ...
        if (sap->local_node_neighbors != NULL) {
            ...
            for (elt = list_get_first_elt(regions); elt != NULL; elt = list_get_next_elt(elt)) {
                ...
                for (contact = get_first_contact_from_node(*currentRegion, localNode, &node);
                     contact != NULL && !stop; contact = get_next_contact(&node)) {
                    if (prevContact != NULL) {
                        if (contact->regionNbr != *currentRegion ||
                            contact->fromNode != localNode ||
                            contact->toNode != prevContact->toNode) {
                            if (contact->regionNbr != *currentRegion ||
                                contact->fromNode != localNode) {
