...
#ifndef WISE_NODE
/**
 * \brief Boolean: set to 1 (enable) if this node has the complete knowledge of the contact plan,
 * otherwise set to 0. Only a "wise node" can update the CGRR extension block.
 *
 * \par Notes:
 ...
 *
 * \details Enabling this macro implies that the BP the BP includes the source code of RGR Ext.
 * Block.
 *
 * \par Notes:
...
/*
 * In this section you can enable some feature of Unibo-CGR in order to get better performance from
 * SABR.
 */
...
/**
 * \brief   This macro limits the number of Dijkstra's routes calculated by the Unibo_CGR
 * "one-route-per-neighbor" enhancement.
 *
 * \details CCSDS SABR leaves to the implementation the number of routes computed by Dijkstra at
 * each call of Phase 1; in ION only one route is computed. Better perfomance can be achived by
 * computing additional routes, in particular by forcing one route for each neighbor, if possible
 * ("one-route- per-neighbor"). To cope with environments with stringent computational constraints,
 * it is possible to limit the number of routes to the best N neighbors.
 *          - Set to 0 to disable the limit (i.e. leaving one-route-per-neigbhor unmodified).
 *          - Set to N (N > 0) to limit the number of routes; in particular, set to 1 to enforce ION
 * behavior.
 *
 * \hideinitializer
 */
...
/**
 * \brief    Unibo_CGR provides the user with both a reactive and a proactive mechanism to
 * counteract loops.
 *
 ...
 *
 * \details This is a possible enhancement provided by Unibo_CGR. In both CCSDS SABR and ION only
 * the local queue delay is computed ("ETO on first hop") However, by exploiting the Expected Volume
 * Consumptions of the route contacts, it is possible to have a rough but conservative estimation of
 ...
 *
 * \details This is a possible enhancement provided by Unibo_CGR. Dijkstra is called in phase 1 to
 * compute a CGR route (sequence of contacts) to destination D. At computation time ("t0") this
 * route, however, because of Dikstra's optimality, it is also the best route to get not only to D,
 * but to all nodes along the geo route subjected at the CGR route (e.g. B-C-F-D). Set to 1 if you
 * want to add the computed route to the computed routes list of each node of the GEO route (e.g.
 * B,C,F and D). Set to 0 if you want the computed route to be added only to D, thus disabling this
 * optimization. Note that this optimization is intended to reduce computational load, should a
 * following bundle be destined (at "t1") to an intermediate node (e.g. B,C or F). On the other
 * hand, a route computed at "t0" may be no more optimal at "t1", with a possible impact on routing
 * accuracy.
...
#ifndef MSR
/**
 * \brief Used to enable or disable Moderate Source Routing: set to 1 to enable MSR, otherwise set
 * to 0.
 *
 * \par Notes:
 ...
 *        The fromTime of the CGRR contact can differs of MSR_TIME_TOLERANCE from the fromTime of
 * the contact stored in the contact graph of this node.
 */
#define MSR_TIME_TOLERANCE 2
#endif
...
/**
 * \brief Used only with WISE_NODE disabled: set to N if you want that this node find (in the
 * contact graph) at least the first N contacts (hops) of the MSR route.
 *
 * \hideinitializer
 */
...
// MSR_HOPS_LOWER_BOUND must be greater than 0.
      ...
      (UNIBO_CGR_SUGGESTED_SETTINGS || MSR_CONFIG || UNIBO_CGR_ENH_WITHOUT_EXP_EXTENSIONS)) ||     \
     (UNIBO_CGR_SUGGESTED_SETTINGS &&                                                              \
      (CCSDS_SABR_DEFAULTS || MSR_CONFIG || UNIBO_CGR_ENH_WITHOUT_EXP_EXTENSIONS)) ||              \
