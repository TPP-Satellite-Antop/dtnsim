...
void record_phases_start_time(UniboCgrPhase phase) {
    ...
    if (found) {
        phasesTime->phase_time_logger[phaseSelected].phase_time.beginOk = clock_gettime(
            CLOCK_REALTIME, &(phasesTime->phase_time_logger[phaseSelected].phase_time.beginTime));
    }
...
void record_total_core_start_time() {
    ...
    totalTime->total_time.beginOk =
        clock_gettime(CLOCK_REALTIME, &(totalTime->total_time.beginTime));
...
void record_total_core_stop_time() {
    ...
    totalTime->timer = (endTime.tv_sec - totalTime->total_time.beginTime.tv_sec) * 1000000000 +
                       endTime.tv_nsec - totalTime->total_time.beginTime.tv_nsec;
...
void print_time_results(time_t currentTime, unsigned int callNumber, CgrBundleID *id) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        copiedPhasesTime.phase_time_logger[i].call_counter =
            phasesTime->phase_time_logger[i].call_counter;
    ...
    written = sprintf(row, "%llu,%ld,%u,%llu,%llu,%u,%u,%u,%llu,%llu,%llu,%u,%llu,%u,%llu,%u\n",
                      get_local_node(), (long int)currentTime, callNumber, id->source_node,
                      id->creation_timestamp, id->sequence_number, id->fragment_length,
...
void initialize_time_analysis() {
    ...
    char file_name[50];
    char *header = "local_node,current_time,call_num,src,ts,sqn_num,fragL,fragO,total_interface,"
                   "total_core,ph_1_time,ph_1_calls,ph_2_time,ph_2_calls,ph_3_time,ph_3_calls\n";

    if (timeFile->configured == 0 || timeFile->fd < 0) {
        sprintf(file_name, "total_%llu.csv", node_eid);
        timeFile->fd =
            open(file_name, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
