 ...
 *                           otherwise set a new CurrentCallSAP (the previous one will be
 *overwritten).
 *
 *
...
static void printDebugIonRoute(PsmPartition ionwm, CgrRoute *route) {
    ...
    if (route != NULL) {
        ...
        fprintf(stdout, "%-15llu %-15ld %-15ld %-15ld %-15ld %-15g %lu\n",
                ...
                (long int)route->pbat - ref, route->maxVolumeAvbl,
                (long unsigned int)route->bundleECCC);
        fprintf(stdout, "%-15s %-15s %-15s %-15s %-15s %s\n", "Confidence", "Hops",
                "Overbooked (G)", "Overbooked (U)", "Protected (G)", "Protected (U)");
        ...
        for (addr = sm_list_first(ionwm, route->hops); addr != 0 && !stop;
            ...
            if (addrContact != 0) {
                ...
                if (contact != NULL) {
                    ...
                    fprintf(stdout, "%-15llu %-15llu %-15ld %-15ld %-15lu %-15.2f %-15g %-15g %g\n",
                            (unsigned long long)contact->fromNode,
                            (unsigned long long)contact->toNode, (long int)contact->fromTime - ref,
                            (long int)contact->toTime - ref, (long unsigned int)contact->xmitRate,
                            contact->confidence, contactBuf.mtv[0], contactBuf.mtv[1],
                            contactBuf.mtv[2]);
                } else {
...
#else
#define printDebugIonRoute(ionwm, route)                                                           \
    do {                                                                                           \
    } while (0)
#endif
 ...
 * \param[in]    reference_time The reference time used to convert POSIX time in differential time
 *from it.
 *
 *
 * \par Revision History:
 ...
 * \param[out]  *resultBlk The GeoRoute extracted from the RGR Extension Block, only in success
 *case. The resultBLk must be allocated by the caller.
 *
 ...
 *****************************************************************************/
static int refill_mtv_into_ion(uint32_t regionNbr, uvast fromNode, uvast toNode, time_t fromTime,
                               unsigned int tolerance, uvast refillSize, int priority,
                               time_t reference_time) {
    Sdr sdr = getIonsdr();
    ...
    if (priority < 0 || priority > 2) {
        ...
        // search the contact
        for (oK(sm_rbt_search(ionwm, ionvdb->contactIndex, rfx_order_contacts, &arg, &elt));
             elt && !stop; elt = sm_rbt_next(ionwm, elt)) {
            contactAddr = sm_rbt_data(ionwm, elt);
            contact = (IonCXref *)psp(ionwm, contactAddr);

            if (contact != NULL && contact->regionNbr == regionNbr &&
 ...
 * \param[in]   reference_time The reference time used to convert POSIX time in differential time
 *from it. \param[in]   regionNbr      Contact plan region
 *
 * \warning bundle    doesn't have to be NULL
 ...
 *  11/12/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int update_mtv_before_reforwarding(Bundle *bundle, ExtensionBlock *cgrrExtBlk,
                                          CGRRouteBlock *cgrrBlk, time_t reference_time,
                                          uint32_t regionNbr) {
    int result = -1;
    uvast size;
    int temp;
        ...
        if (lastRoute != NULL && lastRoute->hopCount > 0) {
            ...
            for (i = 0; i < lastRoute->hopCount; i++) {
                ...
                refill_mtv((unsigned long)regionNbr, (unsigned long long)currentHop->fromNode,
                           (unsigned long long)currentHop->toNode, currentHop->fromTime,
                           timeTolerance, (unsigned int)size, priority);

                // refill MTV into ION's contact graph
                refill_mtv_into_ion(regionNbr, currentHop->fromNode, currentHop->toNode,
                                    currentHop->fromTime, timeTolerance, size, priority,
                                    reference_time);
            }
        }
    }
 ...
 * \param[in]   reference_time The reference time used to convert POSIX time in differential time
 *from it. \param[out]  *extBlk        The ExtensionBlock type of CGRR \param[out] **resultBlk The
 *CGRRouteBlock extracted from the CGRR Extension Block, only in success case. The resultBLk will be
 ...
 *  23/04/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int get_cgrr_ext_block(Bundle *bundle, time_t reference_time, ExtensionBlock *extBlk,
                              CGRRouteBlock **resultBlk) {
    Sdr sdr = getIonsdr();
    int result = 0, i, j;
    Object extBlockElt;
    Address extBlkAddr;
...
static int CGRR_management(PsmPartition ionwm, Lyst bestRoutes, Bundle *bundle) {
    ...
    for (lyst_elt = lyst_first(bestRoutes); lyst_elt && !stop; lyst_elt = lyst_next(lyst_elt)) {
        ...
        if (temp == -2) {
            ...
            temp = saveRouteToExtBlock(cgrr_route.hopCount, cgrr_route.hopList, bundle);
 ...
 * \param[in]    reference_time     The reference time used to convert POSIX time in differential
 *time from it. \param[in]    *IonBundle         The bundle in ION \param[out]   *CgrBundle The
 *bundle in this CGR's implementation.
 *
 * \par	Notes:
 ...
 *****************************************************************************/
static int convert_bundle_from_ion_to_cgr(unsigned long long toNode, time_t current_time,
                                          time_t reference_time, Bundle *IonBundle,
                                          CgrBundle *CgrBundle) {
    int result = -1;
    time_t offset;
    ...
    if (IonBundle != NULL && CgrBundle != NULL) {
        ...
        if (result != -2) {
            ...
            CgrBundle->size =
                NOMINAL_PRIMARY_BLKSIZE + IonBundle->extensionsLength + IonBundle->payload.length;

            CgrBundle->evc = computeBundleEVC(CgrBundle->size); // SABR 2.4.3

            /* Need to convert from msec (since EPOCH 2000) to seconds since 1970 */
            time_t creationTimeInSeconds =
                (time_t)(IonBundle->id.creationTime.msec / 1000) + EPOCH_2000_SEC;

            /* Offset between bundle's creation time and (this) node's start time. */
            offset = creationTimeInSeconds - reference_time;

            /* Internally to Unibo-CGR the bundle's expiration time is relative to the node's start
             * time. Here we make this conversion. */
            ...
            print_log_bundle_id(CgrBundle->id.source_node, CgrBundle->id.creation_timestamp,
                                CgrBundle->id.sequence_number, CgrBundle->id.fragment_length,
 ...
 *
 * \param[in]   reference_time  The reference time used to convert POSIX time in differential time
 *from it. \param[in]   ionwm           The partition of the ION's contacts graph \param[in] *ionvdb
 *The ion's volatile database \param[in]   CgrHops         The list of contacts of this CGR's
 *implementation \param[out]  IonHops         The list of contact in ION's format.
 *
 ...
 *  19/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int convert_hops_list_from_cgr_to_ion(time_t reference_time, PsmPartition ionwm,
                                             IonVdb *ionvdb, List CgrHops, PsmAddress IonHops) {
    ListElt *elt;
    ...
    for (elt = CgrHops->first; elt != NULL && result >= 0; elt = elt->next) {
        if (convert_contact_from_cgr_to_ion((Contact *)elt->data, &IonContact, reference_time) ==
            0) {
            tree_node =
                sm_rbt_search(ionwm, ionvdb->contactIndex, rfx_order_contacts, &IonContact, 0);
            if (tree_node != 0) {
 ...
 *
 * \param[in]    reference_time   The reference time used to convert POSIX time in differential time
 *from it. \param[in]     ionwm           The partition of the ION's contacts graph \param[in]
 **ionvdb         The ION's volatile database \param[in]     *terminusNode   The node for which the
 *routes have been computed \param[in]     evc             Bundle's estimated volume consumption
 ...
 *  19/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int convert_routes_from_cgr_to_ion(time_t reference_time, PsmPartition ionwm, IonVdb *ionvdb,
                                          IonNode *terminusNode, long unsigned int evc,
                                          List cgrRoutes, Lyst IonRoutes) {
    ListElt *elt;
    PsmAddress hops;
    ...
    for (elt = cgrRoutes->first; elt != NULL && result >= 0; elt = elt->next) {
        if (elt->data != NULL) {
            ...
            if (IonRoute != NULL && hops != 0) {
                ...
                tmp = convert_hops_list_from_cgr_to_ion(reference_time, ionwm, ionvdb,
                                                        current->hops, hops);
                if (tmp >= 0) {
                    IonRoute->hops = hops;
                    ...
                    writeLog("(Interface) Skipped route to neighbor %llu, conversion failed.",
                             current->neighbor);
                } else {
                    result = -2;
                }
...
static int add_contact(IonCXref *ContactInION, time_t reference_time) {
    ...
    if (result == 0) {
        ...
        result = addContact(CgrContact.regionNbr, CgrContact.fromNode, CgrContact.toNode,
                            CgrContact.fromTime, CgrContact.toTime, CgrContact.xmitRate,
                            CgrContact.confidence, COPY_MTV, mtv);
        ...
        result = addContact(CgrContact.regionNbr, CgrContact.fromNode, CgrContact.toNode,
                            CgrContact.fromTime, CgrContact.toTime, CgrContact.xmitRate,
                            CgrContact.confidence, DO_NOT_COPY_MTV, mtv);
        if (result >= 1) {
            // result == 2 : xmitRate revised, consider it as "new contact"
            result = 1;
        }
...
static int add_new_contacts(PsmPartition ionwm, IonVdb *ionvdb, time_t reference_time) {
    ...
    for (nodeAddr = sm_rbt_first(ionwm, ionvdb->contactIndex); nodeAddr != 0 && !stop;
         nodeAddr = sm_rbt_next(ionwm, nodeAddr)) {
...
static int add_new_ranges(PsmPartition ionwm, IonVdb *ionvdb, time_t reference_time) {
    ...
    for (nodeAddr = sm_rbt_first(ionwm, ionvdb->rangeIndex); nodeAddr != 0 && !stop;
         nodeAddr = sm_rbt_next(ionwm, nodeAddr)) {
 ...
 *                                 to save the excluded neighbors into the sap->excludedNeighbors
 *list. The excluded neighbors list must be previously allocated by the caller.
 *
 ...
 *****************************************************************************/
int cgr_identify_best_routes(IonNode *terminusNode, Bundle *bundle, Lyst excludedNodes, time_t time,
                             CgrSAP sap, CgrTrace *trace, Lyst IonRoutes) {
    PsmPartition ionwm = getIonwm();
    IonVdb *ionvdb = getIonVdb();
    ...
    if (int_ucgr_sap != NULL && int_ucgr_sap->initialized && bundle != NULL &&
        terminusNode != NULL && ionwm != NULL && ionvdb != NULL && cgrvdb != NULL) {
        start_call_log(time - int_ucgr_sap->reference_time, uniboCgrSAP.count_bundles);

        currentCallSap = get_current_call_sap(NULL);
        // INPUT CONVERSION: check if the contact plan has been changed, in affermative case update
        // it
        ...
        if (result != -2) {
            ...
            if (result == 0) {
                // INPUT CONVERSION: learn the bundle's characteristics and store them into the
                // CGR's bundle struct
                result = convert_bundle_from_ion_to_cgr(
                    terminusNode->nodeNbr, time - int_ucgr_sap->reference_time,
                    int_ucgr_sap->reference_time, bundle, currentCallSap->uniboCgrBundle);
                if (result == 0) {
                    ...
                    if (result >= 0) {
                        ...
                        // Call Unibo-CGR
                        result = getBestRoutes(time - int_ucgr_sap->reference_time,
                                               currentCallSap->uniboCgrBundle,
                                               currentCallSap->excludedNeighbors, &cgrRoutes);

                        if (result > 0 && cgrRoutes != NULL) {
                            // OUTPUT CONVERSION: convert the best routes into ION's CgrRoute and
                            // put them into ION's Lyst
                            result = convert_routes_from_cgr_to_ion(
                                int_ucgr_sap->reference_time, ionwm, ionvdb, terminusNode,
 ...
 * \param[in]   ordinal                    The bundle's ordinal priority (used with expedited
 *cardinal priority) \param[out]  *CgrApplicableBacklog      The applicable backlog computed
 ...
 *  19/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
int computeApplicableBacklog(unsigned long long neighbor, int priority, unsigned int ordinal,
                             CgrScalar *CgrApplicableBacklog, CgrScalar *CgrTotalBacklog) {
    int result = -1;
    Sdr sdr;
