...
typedef enum { NoCRC = 0, X25CRC16 = 1, CRC32C = 2 } BpCrcType;
...
extern int bp_open(char *eid, BpSAP *ionsapPtr);
/*	Arguments are:
...
extern int bp_open_source(char *eid, BpSAP *ionsapPtr, int detain);
/*	Arguments are:
 *  		name of the endpoint
 *		pointer to variable in which
 *			address of BP service
...
extern int bp_parse_quality_of_service(const char *token, BpAncillaryData *ancillaryData,
                                       BpCustodySwitch *custodySwitch, int *classOfService);
...
extern int bp_send(BpSAP sap, char *destEid, char *reportToEid, int lifespan, int classOfService,
                   BpCustodySwitch custodySwitch, unsigned char srrFlags, int ackRequested,
...
extern int bp_track(Object bundleObj, Object trackingElt);
...
extern void bp_untrack(Object bundleObj, Object trackingElt);
/*	Removes trackingElt from the bundle's
 *	list of "tracking" references, if it
 *	is in that list.  Does not delete
 *	trackingElt itself.			*/

extern int bp_memo(Object bundleObj, unsigned int interval);
...
extern int bp_receive(BpSAP sap, BpDelivery *dlvBuffer, int timeoutSeconds);
...
extern void bp_release_delivery(BpDelivery *dlvBuffer, int releaseAdu);
