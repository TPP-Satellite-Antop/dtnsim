...
static int get_rgr_ext_block(dtn::Bundle *bundle, GeoRoute *resultBlk) {
    ...
    // pay attention, resultBlk NEEDS to be allocated ( malloc() ) and then call free() outside this
    // function in convert_bundle_...()
    return result;
}
#endif
 ...
 *
 * \param[in]   *bundle    The DTN2 bundle that contains the RGR Extension Block
 * \param[out] **resultBlk The CGRRouteBlock extracted from the CGRR Extension Block, only in
 *success case. The resultBLk will be allocated by this function.
 *
 * \warning bundle    doesn't have to be NULL
 * \warning resultBlk doesn't have to be NULL
 *
 ...
 *****************************************************************************/
static int convert_bundle_from_dtn2_to_cgr(time_t current_time, dtn::Bundle *Dtn2Bundle,
                                           CgrBundle *CgrBundle) {
    ...
    if (Dtn2Bundle != NULL && CgrBundle != NULL) {
        ...
        if (ipnName.rfind("dtn", 0) == 0) {
            ...
            std::string s =
                ipnName.substr(ipnName.find(delimiter1) + 1, ipnName.find(delimiter2) - 1);
            ...
            if (result != -2) {
                ...
                // offset è la differenza tra la creazione del bundle e il momento di partenza del
                // demone dtnd CgrBundle->expiration_time = IonBundle->expirationTime
                ...
                if (pr < 0 || pr > 2) {
                    CgrBundle->priority_level = Normal;
                    writeLog("Error: priority of bundle is not a valid value: %zu. Using default: "
                             "normal",
                             pr);
                }
 ...
 *****************************************************************************/
static int convert_routes_from_cgr_to_dtn2(long unsigned int evc, List cgrRoutes,
                                           std::string *res) {
    ListElt *elt;
    Route *current;
    size_t count = 0;
...
static int add_contact(char *fileline) {
    ...
    if (result == 0) {
        // Try to add contact
        result = addContact(defaultRegionNbr, CgrContact.fromNode, CgrContact.toNode,
                            CgrContact.fromTime, CgrContact.toTime, CgrContact.xmitRate,
                            CgrContact.confidence, 0, NULL);
        if (result >= 1) {
            result = 1;
        }
...
static int read_file_contactranges(char *filename) {
    ...
    if (fd < 0) {
        ...
        while (read(fd, &car, sizeof(char))) {
            ...
            if (car == '\n') {
                if (skip == 0) {
                    ...
                    if (count > 18 && res[0] == 'a') {
                        if (res[2] == 'c' && res[3] == 'o' && res[4] == 'n' && res[5] == 't' &&
                            res[6] == 'a' && res[7] == 'c' && res[8] == 't') {
                            result = add_contact(res);
                            if (result == 1) {
                                result_contacts++;
                            }
                        } else if (res[2] == 'r' && res[3] == 'a' && res[4] == 'n' &&
                                   res[5] == 'g' && res[6] == 'e') {
        ...
        if (result_ranges > 0) {
            // This piece of code add bidirectional range if there are only unidirectional in the
            // contact plan
            RbtNode *node = NULL;
            Range *primo;
            RbtNode *node2 = NULL;
            ...
            if (primo != NULL) {
                ...
                do {
                    if ((*primo).fromNode < (*primo).toNode) {
                        trovato = get_first_range_from_node_to_node((*primo).toNode,
                                                                    (*primo).fromNode, &node2);
                        if (trovato == NULL) {
                            // I add the opposite range
                            result = addRange((*primo).toNode, (*primo).fromNode, (*primo).fromTime,
                                              (*primo).toTime, (*primo).owlt);
                            if (result >= 1) {
                                result_ranges++;
                            }
                        }
...
int callUniboCGR(time_t time, dtn::Bundle *bundle, std::string *res) {
    ...
    if (initialized && bundle != NULL) {
        ...
        if (result != -2) {
            // INPUT CONVERSION: learn the bundle's characteristics and store them into the CGR's
            // bundle struct
            ...
            if (result == 0) {
                ...
                // Call Unibo-CGR
                result =
                    getBestRoutes(time - reference_time, cgrBundle, excludedNeighbors, &cgrRoutes);

                if (result > 0 && cgrRoutes != NULL) {
                    // OUTPUT CONVERSION: convert the best routes into DTN2's CgrRoute and
                    // put them into ION's Lyst
                    result = convert_routes_from_cgr_to_dtn2(cgrBundle->evc, cgrRoutes, res);
