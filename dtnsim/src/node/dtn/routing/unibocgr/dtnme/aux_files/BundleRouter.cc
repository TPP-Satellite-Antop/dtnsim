...
BundleRouter::Config::Config()
    : type_("static"), add_nexthop_routes_(true), open_discovered_links_(true),
      default_priority_(0), max_route_to_chain_(10), storage_quota_(0), subscription_timeout_(600),
      static_router_prefer_always_on_(true), auto_deliver_bundles_(true) {}

BundleRouter::Config BundleRouter::config_;

//----------------------------------------------------------------------
BundleRouter *BundleRouter::create_router(const char *type) {
...
BundleRouter::BundleRouter(const char *classname, const std::string &name)
    : BundleEventHandler(classname, "/dtn/route"), name_(name) {
...
//----------------------------------------------------------------------
bool BundleRouter::should_fwd(const Bundle *bundle, const LinkRef &link,
    ...
    if (info.state() == ForwardingInfo::TRANSMITTED) {
        log_debug("should_fwd bundle %" PRIbid ": "
                  ...
                  bundle->bundleid(), link->name(), ForwardingInfo::state_to_str(info.state()));
        return false;
    }
    ...
    if (link->remote_eid() != EndpointID::NULL_EID()) {
        size_t count = bundle->fwdlog()->get_count(link->remote_eid(), ForwardingInfo::TRANSMITTED |
                                                                           ForwardingInfo::QUEUED);

        if (count > 0) {
            log_debug("should_fwd bundle %" PRIbid ": "
                      "skip %s since already sent or queued %zu times for remote eid %s",
                      bundle->bundleid(), link->name(), count, link->remote_eid().c_str());
            return false;
        }
        ...
        count = bundle->fwdlog()->get_count(link->remote_eid(), ForwardingInfo::SUPPRESSED);

        if (count > 0) {
            log_debug("should_fwd bundle %" PRIbid ": "
                      "skip %s since transmission suppressed to remote eid %s",
                      bundle->bundleid(), link->name(), link->remote_eid().c_str());
            return false;
        }
    }
    ...
    if (bundle->singleton_dest() && action == ForwardingInfo::FORWARD_ACTION) {
        size_t count = bundle->fwdlog()->get_count(
            ForwardingInfo::TRANSMITTED | ForwardingInfo::QUEUED, action);
...
//----------------------------------------------------------------------
bool BundleRouter::accept_bundle(Bundle *bundle, int *errp) {
...
//----------------------------------------------------------------------
bool BundleRouter::accept_custody(Bundle *bundle) {
    (void)bundle;
    return true;
}

//----------------------------------------------------------------------
bool BundleRouter::can_delete_bundle(const BundleRef &bundle) {
...
//----------------------------------------------------------------------
void BundleRouter::delete_bundle(const BundleRef &bundle) {
    (void)bundle;
}

//----------------------------------------------------------------------
void BundleRouter::tcl_dump_state(oasys::StringBuffer *buf) {
    buf->append("{}");
}

//----------------------------------------------------------------------
void BundleRouter::recompute_routes() {}

//----------------------------------------------------------------------
void BundleRouter::shutdown() {}
