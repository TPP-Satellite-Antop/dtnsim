...
//----------------------------------------------------------------------
UniboCGRBundleRouter::UniboCGRBundleRouter(const char *classname, const std::string &name)
    : BundleRouter(classname, name), reception_cache_(std::string(logpath()) + "/reception_cache",
                                                      1024) // XXX/demmer configurable??
{
    route_table_ = new RouteTable(name);

    // register the global shutdown function
    BundleDaemon::instance()->set_rtr_shutdown(unibo_cgr_router_shutdown, (void *)0);
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::add_route(RouteEntry *entry, bool skip_changed_routes) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::del_route(const EndpointIDPattern &dest) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_changed_routes() {
    // clear the reception cache when the routes change since we might
    // want to send a bundle back where it came from
    reception_cache_.evict_all();
    reroute_all_bundles();
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_event(BundleEvent *event) {
    dispatch_event(event);
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_bundle_received(BundleReceivedEvent *event) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::remove_from_deferred(const BundleRef &bundle, int actions) {
    ...
    for (iter = links->begin(); iter != links->end(); ++iter) {
        ...
        oasys::ScopeLock l(deferred->list()->lock(), "UniboCGRBundleRouter::remove_from_deferred");
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_bundle_transmitted(BundleTransmittedEvent *event) {
    if (BundleDaemon::instance()->shutting_down())
        return;
...
//----------------------------------------------------------------------
bool UniboCGRBundleRouter::can_delete_bundle(const BundleRef &bundle) {
    ...
    if (bundle->fwdlog()->get_count(ForwardingInfo::TRANSMITTED | ForwardingInfo::DELIVERED) <
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::delete_bundle(const BundleRef &bundle) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_bundle_cancelled(BundleSendCancelledEvent *event) {
    if (BundleDaemon::instance()->shutting_down())
        return;
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_route_add(RouteAddEvent *event) {
    add_route(event->entry_);
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_route_del(RouteDelEvent *event) {
    del_route(event->dest_);
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::add_nexthop_route(const LinkRef &link, bool skip_changed_routes) {
...
//----------------------------------------------------------------------
bool UniboCGRBundleRouter::should_fwd(const Bundle *bundle, RouteEntry *route) {
    if (route == NULL)
        return false;
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_contact_up(ContactUpEvent *event) {
    LinkRef link = event->contact_->link();
    ASSERT(link != NULL);
    ASSERT(!link->isdeleted());

    if (!link->isopen()) {
        log_err("contact up(*%p): event delivered but link not open", link.object());
    }
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_contact_down(ContactDownEvent *event) {
    LinkRef link = event->contact_->link();
    ASSERT(link != NULL);
    ASSERT(!link->isdeleted());
    ...
    if (num_queued != 0) {
        ...
        if (iter == reroute_timers_.end()) {
            log_debug("link %s went down with %zu bundles queued, "
                      ...
                      link->name(), num_queued, link->params().potential_downtime_);

            SPtr_RerouteTimer t = std::make_shared<RerouteTimer>(this, link);
            oasys::SPtr_Timer base_sptr = t;
            oasys::SharedTimerSystem::instance()->schedule_in(
                link->params().potential_downtime_ * 1000, base_sptr);
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::reroute_bundles(const LinkRef &link) {
    ASSERT(!link->isdeleted());
    ...
    if (link->state() != Link::UNAVAILABLE) {
        log_warn("reroute timer fired but link *%p state is %s, not UNAVAILABLE", link.object(),
                 Link::state_to_str(link->state()));
        return;
    }

    log_debug("reroute timer fired -- cancelling %zu bundles on link *%p", link->queue()->size(),
              link.object());
    ...
    // the list
    oasys::ScopeLock l(link->queue()->lock(), "UniboCGRBundleRouter::reroute_bundles");
    ...
    while (!link->inflight()->empty()) {
        ...
        // XXX/dz TCPCL can't guarantee that bundle can be immediately removed from the inflight
        // queue ASSERT(! bundle->is_queued_on(link->inflight()));
    }
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_link_available(LinkAvailableEvent *event) {
    LinkRef link = event->link_;
    ASSERT(link != NULL);
    ASSERT(!link->isdeleted());

    // if it is a discovered link, we typically open it
    if (config_.open_discovered_links_ && !link->isopen() && link->type() == Link::OPPORTUNISTIC &&
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_link_created(LinkCreatedEvent *event) {
    LinkRef link = event->link_;
    ASSERT(link != NULL);
    ASSERT(!link->isdeleted());
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_link_deleted(LinkDeletedEvent *event) {
    LinkRef link = event->link_;
    ASSERT(link != NULL);
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_link_check_deferred(LinkCheckDeferredEvent *event) {
    // log_debug("LinkCheckDeferred event for *%p", event->linkref_.object());
    check_next_hop(event->linkref_);
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_custody_timeout(CustodyTimeoutEvent *event) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::get_routing_state(oasys::StringBuffer *buf) {
    buf->appendf("Route table for %s router:\n\n", name_.c_str());
    route_table_->dump(buf);
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::tcl_dump_state(oasys::StringBuffer *buf) {
    oasys::ScopeLock l(route_table_->lock(), "UniboCGRBundleRouter::tcl_dump_state");

    RouteEntryVec::const_iterator iter;
    for (iter = route_table_->route_table()->begin(); iter != route_table_->route_table()->end();
         ++iter) {
        const RouteEntry *e = *iter;
        buf->appendf(" {%s %s source_eid %s priority %d} ", e->dest_pattern().c_str(),
                     e->next_hop_str().c_str(), e->source_pattern().c_str(), e->priority());
    }
}

//----------------------------------------------------------------------
bool UniboCGRBundleRouter::fwd_to_nexthop(Bundle *bundle, RouteEntry *route) {
    bool result = false;
    ...
    if (link->isavailable() && (!link->isopen()) && (!link->isopening())) {
        log_debug("opening *%p because a message is intended for it", link.object());
        actions_->open_link(link);
    }
    ...
    if (!bundle->is_queued_on(deferred->list())) {
        ...
        ForwardingInfo info(ForwardingInfo::NONE, route->action(), link->name_str(), 0xffffffff,
                            link->remote_eid(), route->custody_spec());

        oasys::ScopeLock l(deferred->list()->lock(), "UniboCGRBundleRouter::fwd_to_nexthop");

        deferred->add(bref, info);

        result = true;
    } else {
        log_warn("bundle *%p already exists on deferred list of link *%p", bundle, link.object());
    }
...
//----------------------------------------------------------------------
int UniboCGRBundleRouter::getBacklogForNode(unsigned long long neighbor, int priority,
                                            long int *byteApp, long int *byteTot) {
    // Giacomo:
    ...
    if (matchess.size() > 0) {
        ...
        oasys::ScopeLock l(deferredx->list()->lock(), "computeApplicableBacklog");
        ...
        if (dimLinkQueue > 0) {
            ...
            oasys::ScopeLock l(bl->lock(), "computeApplicableBacklog2");
...
//----------------------------------------------------------------------
int UniboCGRBundleRouter::route_bundle(Bundle *bundle, bool skip_check_next_hop) {
    RouteEntryVec matches;
    RouteEntryVec::iterator iter;
    int result_cgr;

    // check to see if forwarding is suppressed to all nodes
    if (bundle->fwdlog()->get_count(EndpointIDPattern::WILDCARD_EID(), ForwardingInfo::SUPPRESSED) >
        0) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::sort_routes(Bundle *bundle, RouteEntryVec *routes) {
    (void)bundle;
    std::sort(routes->begin(), routes->end(), RoutePrioritySort());
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::check_next_hop(const LinkRef &next_hop) {
    ...
    while (iter != deferred->list()->end()) {
        ...
        if (!BundleRouter::should_fwd(bundle.object(), next_hop, info.action())) {
        ...
        // if the link is available and not open, open it
        if (next_hop->isavailable() && (!next_hop->isopen()) && (!next_hop->isopening())) {
        ...
        actions_->queue_bundle(bundle.object(), next_hop, info.action(), info.custody_spec());
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::reroute_all_bundles() {
    ...
    log_debug("reroute_all_bundles... %zu bundles on pending list", pending_bundles_->size());
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::recompute_routes() {
    reroute_all_bundles();
}

//----------------------------------------------------------------------
UniboCGRBundleRouter::DeferredList::DeferredList(const char *logpath, const LinkRef &link)
    : RouterInfo(), Logger("%s/deferred/%s", logpath, link->name()),
      list_(link->name_str() + ":deferred"), count_(0) {}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::DeferredList::dump_stats(oasys::StringBuffer *buf) {
    buf->appendf(" -- %zu bundles_deferred", count_);
}

//----------------------------------------------------------------------
bool UniboCGRBundleRouter::DeferredList::find(const BundleRef &bundle, ForwardingInfo *info) {
...
//----------------------------------------------------------------------
const ForwardingInfo &UniboCGRBundleRouter::DeferredList::info(const BundleRef &bundle) {
    InfoMap::const_iterator iter = info_.find(bundle->bundleid());
...
//----------------------------------------------------------------------
bool UniboCGRBundleRouter::DeferredList::add(const BundleRef &bundle, const ForwardingInfo &info) {
    ASSERT(list_.lock()->is_locked_by_me());

    if (bundle->is_queued_on(&list_)) {
        log_err("bundle *%p already in deferred list!", bundle.object());
        return false;
    }
...
//----------------------------------------------------------------------
bool UniboCGRBundleRouter::DeferredList::del(const BundleRef &bundle) {
    ASSERT(list_.lock()->is_locked_by_me());
...
//----------------------------------------------------------------------
UniboCGRBundleRouter::DeferredList *UniboCGRBundleRouter::deferred_list(const LinkRef &link) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_registration_added(RegistrationAddedEvent *event) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_registration_removed(RegistrationRemovedEvent *event) {
    (void)event;
}

//----------------------------------------------------------------------
void UniboCGRBundleRouter::handle_registration_expired(RegistrationExpiredEvent *event) {
...
//----------------------------------------------------------------------
void UniboCGRBundleRouter::RerouteTimer::timeout(const struct timeval &now) {
    (void)now;
