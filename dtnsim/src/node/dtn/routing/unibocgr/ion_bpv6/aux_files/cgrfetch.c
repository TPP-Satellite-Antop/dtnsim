...
static void handleTraceState(void *data, unsigned int lineNbr, CgrTraceType traceType,
                             va_list args) {
    ...
    case CgrUpdateRoute:
        ...
        // longer considered.
        for (routeElt = nextConsidered(lyst_first(traceState->routes)); routeElt;
             routeElt = nextConsidered(lyst_next(routeElt))) {
            route = lyst_data(routeElt);
    ...
    case CgrSelectRoutes:
        ...
        traceState->routeElt = nextConsidered(lyst_first(traceState->routes));
        break;
    ...
    case CgrSelectRoute:
        ...
        // considered one.
        traceState->route = lyst_data(traceState->routeElt);
        traceState->routeElt = nextConsidered(lyst_next(traceState->routeElt));
        break;
...
static void traceFnQuiet(void *data, unsigned int lineNbr, CgrTraceType traceType, ...) {
    va_list args;
...
static int contactIsHop(const IonCXref *contact, Route *route) {
    ...
    for (hopElt = lyst_first(route->hops); hopElt; hopElt = lyst_next(hopElt)) {
        hop = lyst_data(hopElt);

        if (contact->fromNode == hop->fromNode && contact->toNode == hop->toNode) {
            return 1;
        }
    }
...
static IonRXref *findRange(const IonCXref *contact) {
    ...
    for (sm_rbt_search(ionwm, ionvdb->rangeIndex, rfx_order_ranges, &arg, &rangeElt); rangeElt;
         rangeElt = sm_rbt_next(ionwm, rangeElt)) {
        range = psp(ionwm, sm_rbt_data(ionwm, rangeElt));

        if (range->fromNode > contact->fromNode || range->toNode > contact->toNode) {
            break;
        }
...
static void output_json(Lyst routes, time_t dispatchTime, time_t expirationTime) {
    PsmPartition ionwm;
    IonVdb *ionvdb;
    uvast localNode;
    ...
    fprintf(outputFile,
            ...
            localNode, destNode, (unsigned int)(dispatchTime), (unsigned int)(expirationTime),
            bundleSize, boolToStr(minLatency));

    for (routeElt = lyst_first(routes), r = 0; routeElt; routeElt = lyst_next(routeElt), r += 1) {
        route = lyst_data(routeElt);
        ...
        for (contactElt = sm_rbt_first(ionwm, ionvdb->contactIndex); contactElt;
             contactElt = sm_rbt_next(ionwm, contactElt)) {
            contact = psp(ionwm, sm_rbt_data(ionwm, contactElt));

            fprintf(f, UVAST_FIELDSPEC " -> " UVAST_FIELDSPEC, contact->fromNode, contact->toNode);

            if (contact->fromTime > route->deliveryTime || contact->toTime < route->fromTime) {
                fputs(" [color = " DISABLED ", fontcolor = " DISABLED "]", f);
            } else if (contactIsHop(contact, route)) {
                fputs(" [color = " HILIGHT ", fontcolor = " HILIGHT "]", f);
            }
        ...
        fprintf(outputFile,
                ...
                route->flag, (unsigned int)(route->fromTime), (unsigned int)(route->deliveryTime),
                route->maxCapacity, route->payloadClass, cgr_reason_text(route->ignoreReason));
...
static void run_cgrfetch(void) {
    ...
    Bundle bundle = {
        .ancillaryData =
            {
                .flags = minLatency ? BP_MINIMUM_LATENCY : BP_BEST_EFFORT,
            },
        .payload =
            {
                .length = bundleSize,
...
static void listOutducts(void) {
    ...
    for (elt = sm_list_first(bpwm, getBpVdb()->outducts); elt; elt = sm_list_next(bpwm, elt)) {
...
static void usage(const char *name) {
    fprintf(stderr,
            ...
            name, (unsigned int)(dispatchOffset), (unsigned int)(expirationOffset), bundleSize,
            outductProto, outductName);
}
...
int cgrfetch(saddr a1, saddr a2, saddr a3, saddr a4, saddr a5, saddr a6, saddr a7, saddr a8,
             saddr a9, saddr a10) {
#else
