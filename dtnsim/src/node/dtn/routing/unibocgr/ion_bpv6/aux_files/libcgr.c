    ...
    if (route->hops) {
        for (citation = sm_list_first(ionwm, route->hops); citation; citation = nextCitation) {
            ...
            if (contact->citations) {
                for (citationElt = sm_list_first(ionwm, contact->citations); citationElt;
...
static int disabledRoute(PsmPartition ionwm, PsmAddress routeElt, PsmAddress *routeAddr,
                         CgrRoute **route) {
    PsmAddress elt;
...
static int getApplicableRange(IonCXref *contact, unsigned int *owlt) {
    ...
    *owlt = 0; /*	Default.			*/
    ...
    for (oK(sm_rbt_search(ionwm, ionvdb->rangeIndex, rfx_order_ranges, &arg, &elt)); elt;
...
static int edgeIsExcluded(PsmPartition ionwm, PsmAddress excludedEdges, PsmAddress contactAddr) {
    PsmAddress elt;

    if (excludedEdges) {
        for (elt = sm_list_first(ionwm, excludedEdges); elt; elt = sm_list_next(ionwm, elt)) {
...
static int computeDistanceToTerminus(IonCXref *rootContact, CgrContactNote *rootWork,
    ...
    while (1) {
        ...
        for (oK(sm_rbt_search(ionwm, ionvdb->contactIndex, rfx_order_contacts, &arg, &elt)); elt;
             elt = sm_rbt_next(ionwm, elt)) {
        ...
        for (elt = sm_rbt_first(ionwm, ionvdb->contactIndex); elt; elt = sm_rbt_next(ionwm, elt)) {
            ...
            if (work->arrivalTime == earliestArrivalTime && work->hopCount >= fewestHops) {
                /*	Not the lowest-cost edge.	*/
    ...
    if (finalContact) /*	Got route to terminal contact.	*/
    {
        ...
        while (contact) {
            ...
            if (sm_list_insert_last(ionwm, contact->citations, citation) == 0) {
                putErrmsg("Can't insert contact into route.", NULL);
                return -1;
            }
...
static int computeRoute(PsmPartition ionwm, PsmAddress rootContactElt, IonNode *terminusNode,
                        time_t currentTime, PsmAddress excludedEdges, PsmAddress *routeAddr,
                        CgrTrace *trace) {
    ...
    *routeAddr = 0;     /*	Default.			*/
    if (rootContactElt) /*	Computing route from waypoint.	*/
    {
        ...
        rootContact = (IonCXref *)psp(ionwm, sm_list_data(ionwm, rootContactElt));
    ...
    if (computeDistanceToTerminus(rootContact, rootWork, terminusNode, currentTime, excludedEdges,
                                  route, trace) < 0) {
        if (route->hops) {
            sm_list_destroy(ionwm, route->hops, NULL, NULL);
        }
    ...
    if (route->toNodeNbr == 0) {
        ...
        *routeAddr = 0;
        ...
        TRACE(CgrProposeRoute, route->toNodeNbr, (unsigned int)(route->fromTime),
...
static int insertFirstRoute(IonNode *terminusNode, time_t currentTime, CgrTrace *trace) {
    PsmPartition ionwm = getIonwm();
    IonVdb *ionvdb = getIonVdb();
    ...
    clearWorkAreas(NULL);
    if (computeRoute(ionwm, 0, terminusNode, currentTime, 0, &routeAddr, trace) < 0) {
    ...
    route = (CgrRoute *)psp(ionwm, routeAddr);
    route->referenceElt = sm_list_insert_last(ionwm, routingObj->selectedRoutes, routeAddr);
...
static int computeSpurRoute(PsmPartition ionwm, IonNode *terminusNode, CgrRoute *lastSelectedRoute,
                            time_t currentTime, PsmAddress rootOfSpur, CgrRtgObject *routingObj,
                            CgrTrace *trace) {
        ...
        while (1) {
            ...
            if (rootPathContactAddr == rootOfSpurAddr) {
                ...
                if (nextContactElt) {
                    ...
                    if (contactAddr) {
                        if (sm_list_insert_last(ionwm, excludedEdges, contactAddr) == 0) {
                            putErrmsg("Can't add \
excluded edge.",
                                      NULL);
                        ...
                        // contact = (IonCXref *) psp(ionwm, contactAddr);
                        // printf("*** Suppressing contact to node " UVAST_FIELDSPEC " after end of
                        // root path. ***\n", contact->toNode);
                    }
                }
    ...
    result = computeRoute(ionwm, rootOfSpur, terminusNode, currentTime, excludedEdges,
                          &newRouteAddr, trace);
    sm_list_destroy(ionwm, excludedEdges, NULL, NULL);
    if (result < 0) {
    ...
    newRoute->referenceElt = sm_list_insert_last(ionwm, routingObj->knownRoutes, newRouteAddr);
...
static int computeAnotherRoute(IonNode *terminusNode, CgrRoute *lastSelectedRoute,
                               time_t currentTime, PsmAddress *elt, CgrTrace *trace) {
    PsmPartition ionwm = getIonwm();
    ...
    *elt = 0; /*	Default: no new route found.		*/
    ...
    while (1) {
        if (computeSpurRoute(ionwm, terminusNode, lastSelectedRoute, currentTime, rootOfSpur,
                             routingObj, trace) < 0) {
    ...
    bestKnownRouteElt = 0;
    for (elt2 = sm_list_first(ionwm, routingObj->knownRoutes); elt2; elt2 = nextRouteElt) {
    ...
    if (bestKnownRouteElt) {
        ...
        *elt = sm_list_insert_last(ionwm, routingObj->selectedRoutes, bestKnownRouteAddr);
        if (*elt == 0) {
...
static time_t computePBAT(CgrRoute *route, Bundle *bundle, time_t currentTime, BpPlan *plan) {
    Sdr sdr = getIonsdr();
    PsmPartition ionwm = getIonwm();
    ...
    for (oK(sm_rbt_search(ionwm, vdb->contactIndex, rfx_order_contacts, &arg, &elt)); elt;
         elt = sm_rbt_next(ionwm, elt)) {
        contact = (IonCXref *)psp(ionwm, sm_rbt_data(ionwm, elt));
    ...
    firstByteTransmitTime += ((ONE_GIG * radiationLatency.gigs) + radiationLatency.units);
    ...
    divideScalar(&radiationLatency, contact->xmitRate);
    lastByteTransmitTime += ((ONE_GIG * radiationLatency.gigs) + radiationLatency.units);
...
float cgr_get_dlv_confidence(Bundle *bundle, CgrRoute *route) {
    ...
    dlvFailureConfidence = (1.0 - bundle->dlvConfidence) * (1.0 - route->arrivalConfidence);
    return (1.0 - dlvFailureConfidence);
}

#if !UNIBO_CGR

static int tryRoute(CgrRoute *route, time_t currentTime, Bundle *bundle, CgrTrace *trace,
                    Lyst bestRoutes) {
    Sdr sdr = getIonsdr();
    ...
    if (candidateElt) /*	May need to replace this one.	*/
    {
        ...
        {
            if (sm_list_length(wm, candidateRoute->hops) < sm_list_length(wm, route->hops)) {
                /*	Current candidate is better.	*/
...
static int checkRoute(IonNode *terminusNode, uvast viaNodeNbr, PsmAddress *elt, Bundle *bundle,
                      Lyst excludedNodes, CgrTrace *trace, Lyst bestRoutes, time_t currentTime,
                      time_t deadline) {
    PsmPartition ionwm = getIonwm();
    PsmAddress nextElt;
    CgrRtgObject *routingObj;
    ...
    } else /*	At end of selectedRoutes list.		*/
    {
        ...
        routingObj = (CgrRtgObject *)psp(ionwm, terminusNode->routingObject);
        for (elt2 = sm_list_last(ionwm, routingObj->selectedRoutes); elt2; elt2 = prevElt2) {
            ...
            if (computeAnotherRoute(terminusNode, route, currentTime, elt, trace)) {
...
static int loadBestRoutesList(IonNode *terminusNode, uvast viaNodeNbr, Bundle *bundle,
    ...
    while (1) {
        switch (checkRoute(terminusNode, viaNodeNbr, &elt, bundle, excludedNodes, trace, bestRoutes,
        ...
        default:
            putErrmsg("Failed checking route to node.", utoa(terminusNode->nodeNbr));
            return -1;
        }
    }
}

static int loadCriticalBestRoutesList(IonNode *terminusNode, Bundle *bundle, Lyst excludedNodes,
    ...
    if (routingObj->proximateNodes == 0) {
        ...
        if (routingObj->proximateNodes == 0) {
            putErrmsg("Can't build list of proximate nodes.", utoa(terminusNode->nodeNbr));
            return -1;
        }
        ...
        for (elt = sm_rbt_first(ionwm, ionvdb->contactIndex); elt; elt = sm_rbt_next(ionwm, elt)) {
            ...
            {
                if (sm_list_insert_last(ionwm, routingObj->proximateNodes,
                                        (PsmAddress)(contact->toNode)) == 0) {
                    putErrmsg("Can't insert prox node.", utoa(terminusNode->nodeNbr));
                    return -1;
                }
            ...
            if (sm_list_insert_before(ionwm, elt2, (PsmAddress)(contact->toNode)) == 0) {
                putErrmsg("Can't insert prox node.", utoa(terminusNode->nodeNbr));
                return -1;
            }
        }
    ...
    for (elt2 = sm_list_first(ionwm, routingObj->proximateNodes); elt2;
         elt2 = sm_list_next(ionwm, elt2)) {
        nodeNbr = (uvast)sm_list_data(ionwm, elt2);
        if (loadBestRoutesList(terminusNode, nodeNbr, bundle, excludedNodes, trace, routes,
        ...
        if (lyst_length(routes) > 0) {
            ...
            if (lyst_insert_last(bestRoutes, lyst_data(lyst_last(routes))) == NULL) {
...
int cgr_identify_best_routes(IonNode *terminusNode, Bundle *bundle, Lyst excludedNodes,
                             time_t currentTime, CgrSAP sap, CgrTrace *trace, Lyst bestRoutes) {
    PsmPartition ionwm = getIonwm();
    CgrVdb *cgrvdb = cgr_get_vdb();
    time_t deadline;
    CgrRtgObject *routingObj;
    ...
    if (routingObj->selectedRoutes == 0) {
        ...
        if (sm_list_insert_last(ionwm, cgrvdb->routingObjects, terminusNode->routingObject) == 0) {
    ...
    if (bundle->ancillaryData.flags & BP_MINIMUM_LATENCY) {
        if (loadCriticalBestRoutesList(terminusNode, bundle, excludedNodes, trace, bestRoutes,
                                       currentTime, deadline, routingObj) < 0) {
            putErrmsg("Can't find all best routes to destination.", utoa(terminusNode->nodeNbr));
            return -1;
        }
    } else {
        if (loadBestRoutesList(terminusNode, 0, bundle, excludedNodes, trace, bestRoutes,
                               currentTime, deadline, routingObj) < 0) {
            putErrmsg("Can't find best route to destination.", utoa(terminusNode->nodeNbr));
            return -1;
        }
    }
...
int cgr_preview_forward(uvast terminusNodeNbr, Bundle *bundle, time_t atTime, CgrSAP sap,
                        CgrTrace *trace) {
    return 0;
}
...
int cgr_preview_forward(uvast terminusNodeNbr, Bundle *bundle, time_t atTime, CgrSAP sap,
    ...
    result = cgr_identify_best_routes(terminusNode, bundle, excludedNodes, atTime, sap, trace,
                                      bestRoutes);
...
int cgr_prospect(uvast terminusNodeNbr, unsigned int deadline) {
    ...
    for (elt = sm_list_first(wm, routingObject->selectedRoutes); elt; elt = nextElt) {
...
const char *cgr_tracepoint_text(CgrTraceType traceType) {
    ...
    static const char *tracepointText[] = {
        [CgrBuildRoutes] = "\n\nBUILD terminusNode:" UVAST_FIELDSPEC " payloadLength:%u atTime:%u",
        [CgrInvalidTerminusNode] = "    INVALID terminus node",

        [CgrBeginRoute] = "  ROUTE",
        [CgrConsiderRoot] = "    ROOT fromNode:" UVAST_FIELDSPEC " toNode:" UVAST_FIELDSPEC,
        [CgrConsiderContact] = "      CONTACT fromNode:" UVAST_FIELDSPEC " toNode:" UVAST_FIELDSPEC,
        [CgrIgnoreContact] = "        IGNORE",

        [CgrCost] = "        COST transmitTime:%u owlt:%u arrivalTime:%u",
        [CgrHop] = "    HOP fromNode:" UVAST_FIELDSPEC " toNode:" UVAST_FIELDSPEC,

        [CgrProposeRoute] =
            "    PROPOSE firstHop to:" UVAST_FIELDSPEC " fromTime:%u arrivalTime:%u",

        [CgrIdentifyRoutes] = "IDENTIFY deadline:%u",
        [CgrFirstRoute] = "    FIRST",
        [CgrNoMoreRoutes] = "    NO MORE",
        [CgrCheckRoute] = "  CHECK firstHop to:" UVAST_FIELDSPEC " fromTime:%u arrivalTime:%u",
