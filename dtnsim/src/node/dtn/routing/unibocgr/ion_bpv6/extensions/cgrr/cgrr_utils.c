...
void printCGRRoute(CGRRoute *cgrRoute) {
    ...
    for (j = 0; j < cgrRoute->hopCount; j++) {
        cgrr_debugPrint("hopList[%d].fromNode = " UVAST_FIELDSPEC ".", j,
                        cgrRoute->hopList[j].fromNode);
        cgrr_debugPrint("hopList[%d].toNode = " UVAST_FIELDSPEC ".", j,
                        cgrRoute->hopList[j].toNode);
        cgrr_debugPrint("hopList[%d].fromTime = %ld.", j,
                        cgrRoute->hopList[j].fromTime - ionReferenceTime(NULL));
    }
...
unsigned char *cgrr_serializeCGRR(uvast *length, CGRRouteBlock *cgrrBlk) {
    ...
    *length += recRoutesLength.length;
    ...
    if ((serializedCgrr = (unsigned char *)MTAKE(*length)) == NULL) {
        cgrr_debugPrint("[x: cgrr_utils/cgrr_serializeCGRR] %d bytes needed but unavailable .",
                        *length);
        return NULL;
    }
...
int cgrr_deserializeCGRR(AcqExtBlock *blk, AcqWorkArea *wk) {
    ...
    // cgrr_debugPrint("i cgrr_deserializeCGRR blk length %u, blk dataLength %u", blk->length,
    // blk->dataLength);
    ...
    // cgrr_debugPrint("i cgrr_deserializeCGRR value in cgrrBlk=%u",
    ...
    if (cgrrBlk.originalRoute.hopCount > 0) {
        ...
        cgrrBlk.originalRoute.hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * cgrrBlk.originalRoute.hopCount);
        if (cgrrBlk.originalRoute.hopList == NULL) {
            cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] not enough space for original "
                            "route Hop List. Need %d bytes.",
                            (sizeof(CGRRHop) * cgrrBlk.originalRoute.hopCount));
        }

        memset((char *)cgrrBlk.originalRoute.hopList, 0,
               (sizeof(CGRRHop) * cgrrBlk.originalRoute.hopCount));
    }
    ...
    for (i = 0; i < cgrrBlk.originalRoute.hopCount; i++) {
        ...
        // cgrr_debugPrint("i cgrr_deserializeCGRR extracted original route H[%d].fromNode of= "
        // UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_deserializeCGRR value in cgrrBlk=
        // " UVAST_FIELDSPEC ".", cgrrBlk.originalRoute.hopList[i].fromNode);
        extractSdnv(&temp, &cursor, &unparsedBytes);
        cgrrBlk.originalRoute.hopList[i].toNode = temp;
        // cgrr_debugPrint("i cgrr_deserializeCGRR extracted original route H[%d].toNode of="
        // UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_deserializeCGRR value in cgrrBlk="
        // UVAST_FIELDSPEC ".", cgrrBlk.originalRoute.hopList[i].toNode);
        extractSdnv(&temp, &cursor, &unparsedBytes);
        cgrrBlk.originalRoute.hopList[i].fromTime = temp;
        // cgrr_debugPrint("i cgrr_deserializeCGRR extracted original route H[%d].fromTime of="
        // UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_deserializeCGRR value in
        // cgrrBlk=%ld.", cgrrBlk.originalRoute.hopList[i].fromTime);
    }

    /* Step 1.3 - If the recomputed legth is greater than zero, we allocate memory to the recomputed
     * routes. else it is not needed, so go on leaving the recomputedRoutes empty. */
    extractSmallSdnv(&itemp, &cursor, &unparsedBytes);
    cgrrBlk.recRoutesLength = itemp;
    cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] extracted recomputed length of=%u",
                    itemp);
    cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] value in cgrrBlk=%u",
                    cgrrBlk.recRoutesLength);
    ...
    if (cgrrBlk.recRoutesLength > 0) {
        ...
        if (cgrrBlk.recomputedRoutes == NULL) {
            cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] not enough space for "
                            "recomputed routes. Need %d bytes.",
    ...
    for (j = 0; j < cgrrBlk.recRoutesLength; j++) {
        ...
        // cgrr_debugPrint("i cgrr_deserializeCGRR extracted recomputed route %d HC of=%u", j,
        // itemp); cgrr_debugPrint("i cgrr_deserializeCGRR value in cgrrBlk=%u",
        // cgrrBlk.recomputedRoutes[j].hopCount);

        // Modified by G.M. De Cola
        cgrrBlk.recomputedRoutes[j].hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * cgrrBlk.recomputedRoutes[j].hopCount);
        if (cgrrBlk.recomputedRoutes[j].hopList == NULL) {
            cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] not enough space for "
                            "recomputed route %d Hop List. Need %d bytes.",
                            j, (sizeof(CGRRHop) * cgrrBlk.recomputedRoutes[j].hopCount));
        }
        memset((char *)cgrrBlk.recomputedRoutes[j].hopList, 0,
               (sizeof(CGRRHop) * cgrrBlk.recomputedRoutes[j].hopCount));

        for (i = 0; i < cgrrBlk.recomputedRoutes[j].hopCount; i++) {
            extractSdnv(&temp, &cursor, &unparsedBytes);
            cgrrBlk.recomputedRoutes[j].hopList[i].fromNode = temp;
            // cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] extracted recomputed route %d
            // H[%d].fromNode of=" UVAST_FIELDSPEC ".",j, i, temp); cgrr_debugPrint("[i:
            // cgrr_utils.c/cgrr_deserializeCGRR] value in cgrrBlk=" UVAST_FIELDSPEC ".",
            // cgrrBlk.recomputedRoutes[j].hopList[i].fromNode);
            extractSdnv(&temp, &cursor, &unparsedBytes);
            cgrrBlk.recomputedRoutes[j].hopList[i].toNode = temp;
            // cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] extracted recomputed route %d
            // H[%d].toNode of=" UVAST_FIELDSPEC ".", j, i, temp); cgrr_debugPrint("[i:
            // cgrr_utils.c/cgrr_deserializeCGRR] value in cgrrBlk=" UVAST_FIELDSPEC ".",
            // cgrrBlk.recomputedRoutes[j].hopList[i].toNode);
            extractSdnv(&temp, &cursor, &unparsedBytes);
            cgrrBlk.recomputedRoutes[j].hopList[i].fromTime = temp;
            // cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] extracted recomputed route %d
            // H[%d].fromTime of= %ld.", j, i, temp - ionReferenceTime(NULL)); cgrr_debugPrint("[i:
            // cgrr_utils.c/cgrr_deserializeCGRR] value in cgrrBlk= %ld.",
            // cgrrBlk.recomputedRoutes[j].hopList[i].fromTime - ionReferenceTime(NULL));
        }
    }

    /* Step 2 - The block has been deserialized. Now we need to tell to the Extension Block the
     * CGRRouteBlock's size and allocate enough memory in the Object scratchpad to store it. */

    // Modified by G.M. De Cola
    // blk->size = sizeof(CGRRouteBlock) + (sizeof(CGRRoute) * cgrrBlk.recRoutesLength) +
    // (sizeof(CGRRHop) * totalHopCount);
    /*	blk->size = sizeof(unsigned int) + sizeof(CGRRoute) + (sizeof(CGRRoute) *
       cgrrBlk.recRoutesLength) + (sizeof(CGRRHop) * totalHopCount);
            //cgrr_debugPrint("[i: cgrr_utils.c/cgrr_deserializeCGRR] size of block set to %u",
       blk->size);

            blk->object = MTAKE(blk->size);
            if (blk->object == NULL)
            {
                    cgrr_debugPrint("[x: cgrr_utils.c/cgrr_deserializeCGRR] No space for
       CGRRouteBlock scratchpad", NULL); return -1;
            }
...
int cgrr_getCGRRFromExtensionBlock(ExtensionBlock *blk, CGRRouteBlock *cgrrBlk) {
    ...
    // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock blk length %u, blk dataLength %u",
    ...
    if (blkBuffer == NULL) {
        cgrr_debugPrint(
            "[cgrr_utils.c/cgrr_getCGRRFromExtensionBlock] No space for block buffer %u.",
            blk->length);
        return -1;
    }
    ...
    cgrr_debugPrint(
        "[i: cgrr_utils.c/cgrr_getCGRRFromExtensionBlock] extracted original route HC of=%u",
        itemp);
    cgrr_debugPrint("[i: cgrr_utils.c/cgrr_getCGRRFromExtensionBlock] value in cgrrBlk=%u",
                    cgrrBlk->originalRoute.hopCount);
    ...
    if (cgrrBlk->originalRoute.hopCount > 0) {
        ...
        cgrrBlk->originalRoute.hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * cgrrBlk->originalRoute.hopCount);
        if (cgrrBlk->originalRoute.hopList == NULL) {
            cgrr_debugPrint("[i: cgrr_utils.c/cgrr_getCGRRFromExtensionBlock] not enough space for "
                            "original route Hop List. Need %d bytes.",
                            (sizeof(CGRRHop) * cgrrBlk->originalRoute.hopCount));
    ...
    for (i = 0; i < cgrrBlk->originalRoute.hopCount; i++) {
        extractSdnv(&temp, &cursor, &unparsedBytes);
        cgrrBlk->originalRoute.hopList[i].fromNode = temp;
        // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted original route H[%d].fromNode
        // of=" UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock
        // value in cgrrBlk=" UVAST_FIELDSPEC ".", cgrrBlk->originalRoute.hopList[i].fromNode);
        // //Modified by F. Marchetti
        extractSdnv(&temp, &cursor, &unparsedBytes);
        cgrrBlk->originalRoute.hopList[i].toNode = temp;
        // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted original route H[%d].toNode
        // of=" UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock
        // value in cgrrBlk=" UVAST_FIELDSPEC ".", cgrrBlk->originalRoute.hopList[i].toNode);
        // //Modified by F. Marchetti
        extractSdnv(&temp, &cursor, &unparsedBytes);
        cgrrBlk->originalRoute.hopList[i].fromTime = temp;
        // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted original route H[%d].fromTime
        // of=" UVAST_FIELDSPEC ".", i, temp); cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock
        // value in cgrrBlk= %ld.", cgrrBlk->originalRoute.hopList[i].fromTime); //Modified by F.
        // Marchetti
    }

    /* Step 1.3 - If the recomputed length is greater than zero, we allocate memory to the
     * recomputed routes. else it is not needed, so go on leaving the recomputedRoutes empty. */
    extractSmallSdnv(&itemp, &cursor, &unparsedBytes);
    cgrrBlk->recRoutesLength = itemp;
    // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted recomputed length of=%u", itemp);
    // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock value in cgrrBlk=%u",
    // cgrrBlk->recRoutesLength); //Modified by F. Marchetti

    /* Step 1.3.1 - In this case, if the recomputed routes length is not greater than zero we can
     * fall through automatically skipping the for cycle, because this may be the second+ node using
     * CGR so there is no recomputed route yet.
     */
    if (cgrrBlk->recRoutesLength > 0) {
        // Modified by G.M. De Cola
        cgrrBlk->recomputedRoutes = (CGRRoute *)MTAKE(sizeof(CGRRoute) * cgrrBlk->recRoutesLength);
    ...
    for (j = 0; j < cgrrBlk->recRoutesLength; j++) {
        ...
        cgrrBlk->recomputedRoutes[j].hopCount = itemp;
        totalHopCount += cgrrBlk->recomputedRoutes[j].hopCount;
        // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted recomputed route %d HC
        // of=%u", j, temp); cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock value in cgrrBlk=%u",
        // cgrrBlk->recomputedRoutes[j].hopCount);

        // Modified by G.M. De Cola
        cgrrBlk->recomputedRoutes[j].hopList = (CGRRHop *)MTAKE(
            sizeof(CGRRHop) * cgrrBlk->recomputedRoutes[j].hopCount); // Modified by F. Marchetti
        if (cgrrBlk->recomputedRoutes[j].hopList == NULL) {
            cgrr_debugPrint("[i: cgrr_utils.c/cgrr_getCGRRFromExtensionBlock] not enough space for "
                            "recomputed route %d Hop List. Need %d bytes.",
                            j, (sizeof(CGRRHop) * cgrrBlk->recomputedRoutes[j].hopCount));
        ...
        for (i = 0; i < cgrrBlk->recomputedRoutes[j].hopCount; i++) {
            extractSdnv(&temp, &cursor, &unparsedBytes);
            cgrrBlk->recomputedRoutes[j].hopList[i].fromNode = temp;
            // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted recomputed route %d
            // H[%d].fromNode of=" UVAST_FIELDSPEC ".",j, i, temp); cgrr_debugPrint("i
            // cgrr_getCGRRFromExtensionBlock value in cgrrBlk=" UVAST_FIELDSPEC ".",
            // cgrrBlk->recomputedRoutes[j].hopList[i].fromNode);
            extractSdnv(&temp, &cursor, &unparsedBytes);
            cgrrBlk->recomputedRoutes[j].hopList[i].toNode = temp;
            // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted recomputed route %d
            ...
            cgrrBlk->recomputedRoutes[j].hopList[i].fromTime = temp;
            // cgrr_debugPrint("i cgrr_getCGRRFromExtensionBlock extracted recomputed route %d
            // H[%d].fromTime of=" UVAST_FIELDSPEC ".", j, i, temp); cgrr_debugPrint("i
            // cgrr_getCGRRFromExtensionBlock value in cgrrBlk=" UVAST_FIELDSPEC ".",
            // cgrrBlk->recomputedRoutes[j].hopList[i].fromTime);
        }
    }
 ...
 * 				 length and a Bundle to which we shall attach the given Hop list if
 *it contains a CGRR extension block. What we do here is create the CGRRoute that will be given to
 *the addRoute() function, which is responsable of actually adding the route.
 *
 * \par Date Written:  22/11/18
 *
 * \return int
 ...
 * 								by the enqueueToNeighbor() function
 *(libcgr.c). \param[in, out] 	bundle  	Pointer to the Bundle struct that represents the
 *bundle to which we shall add the given HopList.
 *
 * \par Notes:
 *      1. This function allocates memory using the MTAKE method.  The
...
int saveRouteToExtBlock(int hopCount, CGRRHop *hopList, Bundle *bundle) {
    ...
    if ((elt = findExtensionBlock(bundle, EXTENSION_TYPE_CGRR, 0, 0, 0)) <= 0) {
        cgrr_debugPrint(
            "[cgrr_utils.c/saveRouteToExtBlock] No CGRR Extesion Block found in bundle.");
        return -2;
    }
    ...
    {
        cgrr_debugPrint(
            "[cgrr_utils.c/saveRouteToExtBlock] cannot instantiate memory for routeToAdd.");
        return -1;
    }
    ...
    {
        cgrr_debugPrint(
            "[cgrr_utils.c/saveRouteToExtBlock] cannot instantiate memory for hopList.");
        MRELEASE(routeToAdd);
        return -1;
    }
    ...
    for (i = 0; i < routeToAdd->hopCount; i++) {
        ...
        cgrr_debugPrint("[cgrr_utils.c/saveRouteToExtBlock] Added routeToAdd.hopList[%d].fromNode "
                        "= " UVAST_FIELDSPEC ".",
                        i, routeToAdd->hopList[i].fromNode);
        routeToAdd->hopList[i].toNode = hopList[i].toNode;
        cgrr_debugPrint("[cgrr_utils.c/saveRouteToExtBlock] Added routeToAdd.hopList[%d].toNode "
                        "= " UVAST_FIELDSPEC ".",
                        i, routeToAdd->hopList[i].toNode);
        routeToAdd->hopList[i].fromTime = hopList[i].fromTime;
        cgrr_debugPrint(
            "[cgrr_utils.c/saveRouteToExtBlock] Added routeToAdd.hopList[%d].fromTime = %ld.", i,
            routeToAdd->hopList[i].fromTime - ionReferenceTime(NULL));
    }
    ...
    if ((result = addRoute(bundle, elt, routeToAdd)) < 0) {
        cgrr_debugPrint(
            "[cgrr_utils.c/saveRouteToExtBlock] an error occurred adding the Route to ExtB.");
    }
 ...
 *
 *
 * \param[in, out] 	bundle  		Pointer to the Bundle struct that represents the
 *bundle to which we shall add the given CGRRoute. \param[in] 		extBlockElt  	Elt Address
 *where we can find the Extension Block. \param[in] 		routeToAdd  	CGRRoute we are
 *going to add to the Extension Block.
 *
 * \par Notes:
 *      1. This function allocates memory using the MTAKE method.  The
 *         block is freed by this function. Any system error will release the memory.
...
int addRoute(Bundle *bundle, Object extBlockElt, CGRRoute *routeToAdd) {
    ...
    if ((result = cgrr_getCGRRFromExtensionBlock(blk, oldCgrrBlk)) < 1) {
        cgrr_debugPrint(
            "addRoute: an error occured retrieving the CGRRouteBlock from the Extension Block.");
    ...
    if (oldCgrrBlk->originalRoute.hopCount == 0) {
        ...
        oldCgrrBlk->originalRoute.hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * routeToAdd->hopCount);

        if (oldCgrrBlk->originalRoute.hopList == NULL) {
            cgrr_debugPrint(
                "[cgrr_utils.c/addRoute] cannot instantiate memory for original route hopeList.");
            releaseCgrrBlkMemory(oldCgrrBlk);
            return -1;
        }
        ...
        if ((result = cgrr_attach(blk, oldCgrrBlk)) < 0) {
            cgrr_debugPrint("[cgrr_utils.c/addRoute] a problem occurred attaching CGRRouteBlock to "
                            "ExtensionBlock.");
        }
    } else {
        ...
        if (newCgrrBlk == NULL) {
            cgrr_debugPrint(
                "[cgrr_utils.c/addRoute] cannot instantiate memory for newCGRRouteBlock.");
            return -1;
        }

        newCgrrBlk->originalRoute.hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * oldCgrrBlk->originalRoute.hopCount);
        newCgrrBlk->recomputedRoutes =
            (CGRRoute *)MTAKE(sizeof(CGRRoute) * (oldCgrrBlk->recRoutesLength + 1));

        if (newCgrrBlk->originalRoute.hopList == NULL || newCgrrBlk->recomputedRoutes == NULL) {
            cgrr_debugPrint("[cgrr_utils.c/addRoute] cannot instantiate memory for original "
                            "route's hop list or recomputed routes.");
            releaseCgrrBlkMemory(newCgrrBlk);
            return -1;
        }

        for (i = 0; i < oldCgrrBlk->recRoutesLength; i++) {
            newCgrrBlk->recomputedRoutes[i].hopList =
                (CGRRHop *)MTAKE(sizeof(CGRRHop) * oldCgrrBlk->recomputedRoutes[i].hopCount);
            if (newCgrrBlk->recomputedRoutes[i].hopList == NULL) {
                cgrr_debugPrint("[cgrr_utils.c/addRoute] cannot instantiate memory for %dth "
                                "recomputedRoute's hop list.",
                                i);
                releaseCgrrBlkMemory(newCgrrBlk);
                return -1;
            }
        ...
        newCgrrBlk->recomputedRoutes[newIdx].hopList =
            (CGRRHop *)MTAKE(sizeof(CGRRHop) * routeToAdd->hopCount);
        if (newCgrrBlk->recomputedRoutes[newIdx].hopList == NULL) {
            cgrr_debugPrint("[cgrr_utils.c/addRoute] cannot instantiate memory for "
                            "newCGRRouteBlock's recomputedRoute to add.");
            releaseCgrrBlkMemory(newCgrrBlk);
            return -1;
        }

        /* Step 2.2: Now that we know that we have enough memory we can check if the sdr can save
         * the new CGRRouteBlock once it will be populated. */

        /* Step 2.3: Now we go ahead and populate the new Extension Block with the retrieved values
         * 			 and then we add the latest recomputed route and increment the
         * recRoutesLength. */
...
static void adjustDbOverhead(Bundle *bundle, unsigned int oldLength, unsigned int newLength,
...
int processModifiedExtensionBlock(Bundle *bundle, Object blkAddr, ExtensionBlock *blk,
    ...
    for (i = 0; i < 2; i++) {
        ...
        for (elt = sdr_list_first(bpSdr, bundle->extensions[i]); elt; elt = nextElt) {
            ...
            if (sdr_list_data(bpSdr, elt) == blkAddr) {
                ...
                if (blk->length != oldLength || blk->size != oldSize) {
                    ...
                    adjustDbOverhead(bundle, oldLength, blk->length, oldSize, blk->size);
                ...
                // CHKERR(sdr_begin_xn(bpSdr));
                sdr_write(bpSdr, blkAddr, (char *)blk, sizeof(ExtensionBlock));
                // sdr_end_xn(bpSdr);
                break;
            }
        }
    ...
    if (bundle->dbOverhead != oldDbOverhead) {
        ...
        zco_increase_heap_occupancy(bpSdr, bundle->dbOverhead, bundle->acct);
        // cgrr_debugPrint("processModifiedExtensionBlock: done.");
    }
...
void copyCGRRoute(CGRRoute *dest, CGRRoute *src) {
    ...
    // cgrr_debugPrint("[cgrr_utils.c/copyCGRRoute] added dest HC=%u.", dest->hopCount);//Modified
    // by F. Marchetti
...
unsigned int writeCGRRouteBlockToSdr(Sdr sdr, Object destAddress, CGRRouteBlock *src)
{
        ...
        CHKERR(sdr_begin_xn(sdr));
        sdr_write(sdr, destAddress + offset, (char*)&(src->recRoutesLength), sizeof
(src->recRoutesLength)); offset += sizeof (src->recRoutesLength); sdr_end_xn(sdr);

        for (i = 0; i < src->recRoutesLength; i++)
        {
                offset += writeCGRRouteToSdr(sdr, destAddress + offset,
&(src->recomputedRoutes[i]));
        }
...
unsigned int writeCGRRouteToSdr(Sdr sdr, Object destAddress, CGRRoute *src)
{
        ...
        for (j = 0; j < src->hopCount; j++)
        {
                ...
                offset += sizeof (Hop);
                sdr_write(sdr, destAddress + offset, (char*)&(src->hopList[j].toNode), sizeof
(src->hopList[j].toNode)); offset += sizeof (src->hopList[j].toNode); sdr_write(sdr, destAddress +
offset, (char*)&(src->hopList[j].fromTime), sizeof (src->hopList[j].fromTime)); offset += sizeof
(src->hopList[j].fromTime);
        }
...
int getCGRRoute(CgrRoute *in_route, CGRRoute *out_route) {
    ...
    for (elt = sm_list_first(ionwm, in_route->hops); elt; elt = sm_list_next(ionwm, elt)) {
