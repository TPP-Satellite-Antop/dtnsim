...
int rgr_attach(Bundle *bundle, ExtensionBlock *blk, GeoRoute *rgrBlk) {
    ...
    if (rgrBlk->length) {
        ...
        // rgr_debugPrint("i rgr_attach encoded length in SDNV. New blk->dataLength: %u",
        // blk->dataLength);
...
void rgr_release(ExtensionBlock *blk) {
    ...
    // rgr_debugPrint("rgr_release: Releasing Register Route sdr memory...(%lu)(%u)", blk->object,
    // blk->size);
 ...
 *  		 |				  | This was necessary due to an apparent ION bug
 *for |				  | which the processOnTransmit doesn't actually attach |
...
int rgr_processOnDequeue(ExtensionBlock *blk, Bundle *bundle, void *ctxt) {
    ...
    if (rgrBlk.length > 0) {
         ...
         * In this case rgrBlk.length must be 0 in order to override "temporaryString" otherwise
         * "temporaryString" is stored in the extension and carried out on the other nodes as part
         * of the extension string In my tests there was a situation like this:
         * temporaryStringipn:1.0,timestamp; etc...
         */
    ...
    if ((blk->object = sdr_malloc(sdr, blk->size)) <= 0) {
        rgr_debugPrint(
            "[rgr.c/rgr_processOnDequeue] Not enough space in sdr to save new block of size %u.",
            rgrBlk.length);
        blk->tag1 = 1;
        blk->size = 0;
        sdr_end_xn(sdr);
...
int rgr_acquire(AcqExtBlock *blk, AcqWorkArea *wk) {
    ...
    //		(unsigned long) wk);
    // rgr_debugPrint("i rgr_acquire: bytes %u length %u dataLength %u", blk->bytes, blk->length,
    // blk->dataLength);
