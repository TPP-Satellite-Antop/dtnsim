 ...
 * 	      tsIntervalDuration: The constant duration of the timestamps, -1 if they differ in
 * length useUncertainty: Indicator, whether to use OCGR or OCGR-UCoP repetition: number of run in
 * the simulations numOfNodes: number of nodes in the network
 *
 * @author Simon Rink
 */
...
void RoutingUncertainUniboCgr::routeAndQueueBundle(BundlePkt *bundle, double simTime) {
    ...
    {
        this->metricCollector_->updateStartedBundles(eid_, bundle->getBundleId(),
                                                     bundle->getSourceEid(),
    ...
    if (useUncertainMode && this->nodeBrufFunction_.find(currDest) ==
                                this->nodeBrufFunction_.end()) // no routing decisions yet
    {
        this->callToPython();
        brufFunction = this->nodeBrufFunction_[currDest];
    } else if (useUncertainMode &&
               this->lastTimeUpdated_[bundle->getDestinationEid()] <
                   this->contactPlan_->getLastEditTime().dbl()) // routing information outdated
    {
        this->callToPython();
        brufFunction = this->nodeBrufFunction_[currDest];
    } else {
        ...
        for (elt = cgrRoutes->first; elt != NULL && result >= 0; elt = elt->next) {
            ...
            if (elt->data != NULL) {
                ...
                if (numberOfRoutes == 1) {
                    ...
                    bundle->setDlvConfidence(
                        bundle->getDlvConfidence() +
                        (1 - (1 - route->arrivalConfidence) * (1 - bundle->getDlvConfidence())));
                    ...
                    bundleCopy->setDlvConfidence(bundleCopy->getDlvConfidence() +
                                                 (1 - (1 - route->arrivalConfidence) *
                                                          (1 - bundleCopy->getDlvConfidence())));
...
void RoutingUncertainUniboCgr::enqueueBundle(BundlePkt *bundle, double simTime, Route *route) {
    ...
    if (route == NULL) {
        ...
        RoutingUncertainUniboCgr *other =
            check_and_cast<RoutingUncertainUniboCgr *>(dtn->getRouting());
        Contact *contactDtnSim =
            this->contactPlan_->getContactBySrcDstStart(this->eid_, destination, simTime);
        ...
        this->multiHops[bundle->getBundleId()].erase(
            this->multiHops[bundle->getBundleId()].begin());

        if (this->multiHops[bundle->getBundleId()].size() > 0) // still multi hops left
        {
            other->notifyAboutMultiHop(this->multiHops[bundle->getBundleId()],
                                       bundle->getBundleId());
        }

        this->multiHops.erase(bundle->getBundleId());
        this->metricCollector_->updateSentBundles(eid_, destination, simTime,
                                                  bundle->getBundleId());
    ...
    if (hops.size() > 1) // multi hop from this source!
    {
        ...
        RoutingUncertainUniboCgr *other =
            check_and_cast<RoutingUncertainUniboCgr *>(dtn->getRouting());
        Contact *contactDtnSim =
            this->contactPlan_->getContactBySrcDstStart(this->eid_, destination, start);
    ...
    if (contactDtnSim->isPredicted() ||
        contactDtnSim->isDiscovered()) // check if a predicted neighbor is actually happening
    {
        ...
        this->metricCollector_->updateSentBundles(eid_, destinationEid, start,
                                                  bundle->getBundleId());
 ...
 * @author Simon Rink
 */
int RoutingUncertainUniboCgr::convertBundlePktToCgrBundle(time_t time, BundlePkt *bundle,
                                                          CgrBundle *cgrBundle) {
...
int RoutingUncertainUniboCgr::addContactToSap(Contact *contact) {
    int result =
        addContact(this->defaultRegionNbr_, contact->getSourceEid(), contact->getDestinationEid(),
                   contact->getStart(), contact->getEnd(), contact->getDataRate(),
                   contact->getConfidence(), 0, NULL, contact->getFailureProbability(), false);

    if (result <= 0) {
        return result;
    }

    result = addRange(contact->getSourceEid(), contact->getDestinationEid(), contact->getStart(),
                      contact->getEnd(), contact->getRange());
...
int RoutingUncertainUniboCgr::removeContactFromSap(Contact *contact) {
    ...
    result = removeContact(this->defaultRegionNbr_, contact->getSourceEid(),
                           contact->getDestinationEid(), NULL);
 ...
 *
 * @param contact: NULL, if all recently added contact should be added to the SAP, a pointer to the
 * contact if it needs to be removed
 *
 * @author Simon Rink
 */
void RoutingUncertainUniboCgr::updateContactPlan(Contact *c) {
    ...
    set_time_contact_plan_updated(cpSAP.contactPlanEditTime.tv_sec,
                                  cpSAP.contactPlanEditTime.tv_usec);
...
int RoutingUncertainUniboCgr::callUniboCGR(time_t time, BundlePkt *bundle, List *cgrRoutes) {
    ...
    if (this->initialised_ && bundle != NULL) {
        ...
        if (result >= 0) {
            ...
            result = getBestRoutes(time - this->referenceTime_, this->cgrBundle_,
                                   this->excludedNeighbors_, cgrRoutes);
 ...
 */
void RoutingUncertainUniboCgr::successfulBundleForwarded(long bundleId, Contact *contact,
...
/*
 * Bundles queued to this contact are examined and then sent to it. Recompute values if it is a
 * discovered contact.
 *
...
void RoutingUncertainUniboCgr::contactStart(Contact *c) {
    ...
    for (size_t i = 0; i < toBeRerouted.size(); i++) {
        ...
        if (this->bundleReroutable.find(bundle->getBundleId()) == this->bundleReroutable.end()) {
            ...
            bool reRoutable =
                this->bundleReroutable[bundle->getBundleId()].front() < simTime().dbl();
...
int getTsForStartOrCurrentTime(int startOrCurrent) {
    ...
    for (size_t i = 0; i < tsStartTimes[currDest].size() - 1; i++) {
        if (startOrCurrent == tsStartTimes[currDest].at(i) ||
            startOrCurrent < tsStartTimes[currDest].at(i + 1)) {
            return i;
        }
    }

    std::cerr << "Error: No timestamp found for time " << startOrCurrent << " at destination "
              << currDest << endl;
    return -1;
}
...
int getTsForEndTime(int end) {
    ...
    std::cerr << "Error: No timestamp found for end time " << end << " at destination " << currDest
              << endl;
    return -1;
}
 ...
 */
double get_probability_if_this_contact_is_chosen(UniboContact *contact,
    ...
    double successProbability =
        (1 - contact->pf) * getNodeFutureDeliveryProbability(contact->toNode, ts + 1);

    successProbability =
        successProbability +
        (contact->pf * getNodeFutureDeliveryProbability(contact->fromNode, ts + 1));
...
void updateStartTimes(vector<Contact> *contacts) {
    ...
    sort(tsStartTimes[currDest].begin(),
         tsStartTimes[currDest].begin() + tsStartTimes[currDest].size()); // the list must be sorted
    numOfTs = findMaxTs();
}
