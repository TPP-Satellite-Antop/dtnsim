...
BundlePkt *CustodyModel::bundleWithCustodyRequestedArrived(BundlePkt *bundleInCustody) {
    ...
    if (sdr_->isSdrFreeSpace(bundleInCustody->getByteLength()) ||
        this->eid_ == bundleInCustody->getDestinationEid()) {
        // Accept custody, send custody report to previous custodian
        cout << simTime() << " Node " << eid_ << " ** Custody accept for bundleId "
             << bundleInCustody->getBundleId() << " - ";
        ...
        cout << simTime() << " Node " << eid_ << " ** Custody reject for bundleId "
             << bundleInCustody->getBundleId() << " - ";
...
BundlePkt *CustodyModel::custodyReportArrived(BundlePkt *custodyReport) {
    ...
    if (custodyReport->getCustodyAccepted()) {
        ...
        cout << simTime() << " Node " << eid_ << " ** Releasing custody of bundleId "
             << custodyReport->getCustodyBundleId() << endl;
        ...
        cout << simTime() << " Node " << eid_ << " ** NOT releasing custody of bundleId "
             << custodyReport->getCustodyBundleId() << " rejected... do something!" << endl;
        reSendBundle =
            sdr_->getTransmittedBundleInCustody(custodyReport->getCustodyBundleId())->dup();
        sdr_->removeTransmittedBundleInCustody(custodyReport->getCustodyBundleId());
...
BundlePkt *CustodyModel::custodyTimerExpired(CustodyTimout *custodyTimout) {
    ...
    if (reSendBundle != NULL) {
        cout << simTime() << " Node " << eid_ << " ** Resending bundleId "
             << custodyTimout->getBundleId() << endl;
...
BundlePkt *CustodyModel::getNewCustodyReport(bool accept, BundlePkt *bundleInCustody) {
    ...
    // Bundle properties
    char bundleName[100];
    sprintf(bundleName, "Src:%d,Dst:%d(id:%d)", this->eid_, bundleInCustody->getCustodianEid(),
            (int)custodyReport->getId());
